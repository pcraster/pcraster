...
template <> ReadOnlyInt4 *spatialInterface<INT4, MAP_INT4>(const MAP_INT4 *o)
{
  switch (o->inCellRepr) {
    case CR_INT4:
...
template <> ReadOnlyReal8 *spatialInterface<REAL8, MAP_REAL8>(const MAP_REAL8 *o)
{
  switch (o->inCellRepr) {
    case CR_REAL4:
...
template <class UseAsT, class OldMapApiT>
const fieldapi::ReadOnly<UseAsT> &fieldapi::UpgradeReadOnly<UseAsT, OldMapApiT>::readOnly()
{
  return *d_ro;
}
...
template <> ReadWriteInt4 *spatialInterface<INT4, MAP_INT4>(MAP_INT4 *o)
{
  switch (o->inCellRepr) {
    case CR_INT4:
...
template <> ReadWriteReal8 *spatialInterface<REAL8, MAP_REAL8>(MAP_REAL8 *o)
{
  switch (o->inCellRepr) {
    case CR_REAL4:
...
//! return new interface
template <class UseAsT, class OldMapApiT>
fieldapi::ReadWrite<UseAsT> &fieldapi::UpgradeReadWrite<UseAsT, OldMapApiT>::readWrite()
{
  return *d_ro;
}
