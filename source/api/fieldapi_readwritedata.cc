...
fieldapi::ReadWriteData<UseAsT, StoredAsT>::ReadWriteData(StoredAsT **data, size_t nrRows, size_t nrCols)
    : ReadWrite<UseAsT>(nrRows, nrCols), d_ro(data, nrRows, nrCols)
{
}
...
template <class UseAsT, class StoredAsT>
void fieldapi::ReadWriteData<UseAsT, StoredAsT>::putMV(size_t rowIndex, size_t colIndex)
{
#ifdef DEBUG_DEVELOP
  PRECOND(!d_ro.outOfRange(rowIndex, colIndex));
#endif
...
template <class UseAsT, class StoredAsT> void fieldapi::ReadWriteData<UseAsT, StoredAsT>::putAllMV()
{
  pcr::setMV(d_ro.d_data[0], this->nrRows() * this->nrCols());
}

template <class UseAsT, class StoredAsT>
bool fieldapi::ReadWriteData<UseAsT, StoredAsT>::isMV(const geo::CellLoc &l) const
{
