...
static void PutAllMVINT4(MAP_INT4 *m)
{
  int r = 0;
  int c = 0;
  PRECOND(m->spatial);
...
static int NrRowsINT4(const MAP_INT4 *m)
{
  return (m->nrRows);
}

static int NrColsINT4(const MAP_INT4 *m)
{
  return (m->nrCols);
}
...
static GET_TEST GetGetTestINT4(const MAP_INT4 *m)
{
  return m->getType;
}

static int HintNrFastListINT4(const MAP_INT4 *m)
{
#ifdef DEBUG
  PRECOND(CR_INT4 != CR_REAL8); /* using on scalar is nonsense */
#endif
  return MIN(MAX_NR_FAST_LIST, m->maxVal);
}

static REAL8 SideMap(const MAP_INT4 *argNeverUsed)
{
...
void DeleteInternalMAP_INT4(MAP_INT4 *m)
