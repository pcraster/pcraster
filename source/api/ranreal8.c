...
#define INSIDE_MAP(thisRow, thisCol, m)                                                                 \
  (0 <= (thisRow) && (thisRow) < ((m)->nrRows) && 0 <= (thisCol) && (thisCol) < ((m)->nrCols))
/*********************/
/* LOCAL DEFINITIONS */
/*********************/
...
/* tests if [r,c] is in map
   ...
   *         FALSE otherwise
   */
static void SpatialAndCoordInMapREAL8(int r,                  /* row number */
                                          int c,                  /* column number */
...
static bool GetSpatialREAL8(REAL8 *v,           /* write-only. Value */
                                int r,                  /* row number */
                                int c,                  /* column number */
                                const MAP_REAL8 *m) /* map this function belongs to */
{
...
static bool GetSpatialMVtestREAL8(REAL8 *v,           /* write-only. Value */
                                      int r,                  /* row number */
                                      int c,                  /* column number */
                                      const MAP_REAL8 *m) /* map this function belongs to */
{
...
static void PutMV_REAL8(int r,            /* row number */
                            int c,            /* column number */
...
static void PutAllMVREAL8(MAP_REAL8 *m)
{
  int r = 0;
  int c = 0;
  PRECOND(m->spatial);
...
static void PutSpatialREAL8(REAL8 v,    /* Value */
                                int r,            /* row number */
                                int c,            /* column number */
                                MAP_REAL8 *m) /* map this function belongs to */
{
...
static int NrRowsREAL8(const MAP_REAL8 *m)
{
  return (m->nrRows);
}

static int NrColsREAL8(const MAP_REAL8 *m)
{
  return (m->nrCols);
}

/* ARGSUSED */
static void PutNonSpatialREAL8(REAL8 v,    /* Value */
                                   int r,            /* row number */
                                   int c,            /* column number */
                                   MAP_REAL8 *m) /* map this function belongs to */
{
...
/* ARGSUSED */
static void PutNonSpatialMV_REAL8(int r,            /* row number */
                                      int c,            /* column number */
                                      MAP_REAL8 *m) /* map this function belongs to */
{
  (void)m;  // Shut up compiler
...
static bool GetNonSpatialREAL8(REAL8 *v,           /* write-only. Value */
                                   int r,                  /* row number */
                                   int c,                  /* column number */
                                   const MAP_REAL8 *m) /* map this function belongs to */
{
 ...
 */
static void SetGetTestREAL8(GET_TEST t,   /* new get test */
...
static GET_TEST GetGetTestREAL8(const MAP_REAL8 *m)
{
  return m->getType;
}

static int HintNrFastListREAL8(const MAP_REAL8 *m)
{
#ifdef DEBUG
  PRECOND(CR_REAL8 != CR_REAL8); /* using on scalar is nonsense */
#endif
  return MIN(MAX_NR_FAST_LIST, m->maxVal);
}

static REAL8 SideMap(const MAP_REAL8 *argNeverUsed)
{
 ...
 */
MAP_REAL8 *InitMapREAL8(size_t r,     /* number of rows */
...
void DeleteInternalMAP_REAL8(MAP_REAL8 *m)
