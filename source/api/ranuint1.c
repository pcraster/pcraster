...
static void PutAllMVUINT1(MAP_UINT1 *m)
{
  int r = 0;
  int c = 0;
  PRECOND(m->spatial);
...
static int NrRowsUINT1(const MAP_UINT1 *m)
{
  return (m->nrRows);
}

static int NrColsUINT1(const MAP_UINT1 *m)
{
  return (m->nrCols);
}
 ...
 */
static void
SetGetTestUINT1(GET_TEST t,               /* new get test */
...
static GET_TEST GetGetTestUINT1(const MAP_UINT1 *m)
{
  return m->getType;
}
...
static REAL8 SideMap(const MAP_UINT1 *argNeverUsed)
{
...
void DeleteInternalMAP_UINT1(MAP_UINT1 *m)
