 ...
 */
int AppCheckVal(const char *v,     /* token value to read */
                                   ...
                                   */
{
    ...
    switch (valueScale) {
    case VS_BOOLEAN:
        result = (CnvrtUINT1(&uint1Val, v) && (uint1Val == 0 || uint1Val == 1));
        break;
    case VS_NOMINAL:
    case VS_ORDINAL:
        if (appLarge)
    ...
    case VS_LDD:
        result =
            (CnvrtUINT1(&uint1Val, v) && (UINT1)0 < uint1Val && uint1Val <= (UINT1)LDD_MAX);
        break;
#ifdef NEVER
 ...
 * Returns 1 in case of an illegal value for valueScale, 0 otherwise.
 */
int AppCheckValNum(REAL8 v,           /* token value to read */
                   CSF_VS valueScale, /* type 2 value scale or VS_UNDEFINED */
                   CSF_CR cellRepr)   /* cell representation, AppDefaultCellRepr()
                                       * is used if this argument has value
                                       * CR_UNDEFINED
