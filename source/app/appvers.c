...
//  */
// int LimitedVersionCheck(int nrRows,       /* number of rows limit: 60 */
...
//     if ((nrRows > 0) && (nrCols > 0) && (nrSteps > 0) &&
//         ((nrRows * nrCols * nrSteps) > 240000)) {
//         int i = nrRows * nrCols * nrSteps;
//         return RetError(
//             1, "%s nr.timesteps*nr.cells may not exceed 240000 (not '%d')", MSG, i);
//     }
//     if (nrLookupRecs > 20)
//         return RetError(
//             1, "%s only 20 records\\tuples allowed in a table (not '%d')", MSG, nrLookupRecs);
//     if (nrXYZRecs > 3600)
//         return RetError(
//             1,
//             "%s only 3600 records\\tuples allowed in point data column file (not '%d')",
//             MSG,
//             nrXYZRecs);
