...
#define NR_COL_MISMATCH "There are %u columns on line %u, %u required"
/**********************/
/* LOCAL DEFINITIONS  */
/**********************/
...
static void InitAllRecs(size_t nrColsReq)
{
  allRecList = NULL;
  nrAllRecords = 0;
  nrAllRecList = 0;
 ...
 */
static void FreeColumnOrTimeSeriesData(double **dataRecords, /* data returned through the first argument
                         * (recs) of a call to AppReadTimeSeriesFile
                         * or AppReadColumnFile
                         */
 ...
 */
void AppFreeColumnData(REAL8 **columnData, /* destructed. Data returned by
                        * AppReadColumnFile
                        */
 ...
 */
void AppFreeTimeSeriesData(REAL8 **timeSeriesData, /* destructed. Data returned by
...
static double *NewAllRec(void)
{
  if (nrAllRecords == nrAllRecList) {
    ...
    size_t i = 0;
    r = (double **)ChkRealloc(allRecList, sizeof(double *) * (nrAllRecList + REC_ALLOC));
...
static int ColError(size_t colNameIndex, size_t colNr, size_t nrCols)
{
  if (nrCols <= colNr) {
    ErrorNested("%s column '%u' too big, nr. of columns in file is '%u'", colName[colNameIndex],
                colNr + 1, nrCols);
    return 1;
  }
  return 0;
...
static int HandleAnErrorDefault(REAL8 *retVal, size_t colNr, size_t lineNr, const char *wrong)
{
...
static int HandleAnErrorTss(REAL8 *retVal, size_t colNr, size_t lineNr, const char *wrong)
{
 ...
 */
static int ReadAllColumnFile(double ***recs,          /* write-only array of nrRecs ptrs to arrays
  ...
  *nrMVcoordColumn = 0;
  ...
  if (*geoeas) {
    if (LexSkipLines((int)(nCols + 2)) != (int)(nCols + 2)) {
      ErrorNested("There are not '%u' lines in the GeoEas header", nCols + 2);
      goto error;
    }
  }
    ...
    for (i = 1; i <= nCols; i++) {
      ...
      switch (token) {
        ...
        case LEX_TOKEN_TOBIG:
          ErrorNested("value '%s' for column %u on line %d is too long", v, i, LexGetLineNr());
          goto error;
        default: /* a column value or empty column */
          if (token == sepChar) {
            /* empty column */
            ErrorNested("column %u on line %d is empty", i, LexGetLineNr());
            goto error;
          }
          /* a column value */
          PRECOND(token == LEX_ILL_TOKEN || token == LEX_NUMBER);
          if (i == 1) { /* first col of new line, check and get a record (r)
                 */
 ...
 * Returns 1 in case of error, 0 otherwise.
 */
int AppReadColumnFile(REAL8 ***recs,           /* write-only array of nrRecs ptrs to arrays
 ...
 * Returns 1 in case of error, 0 otherwise.
 */
int AppReadTimeSeriesFile(REAL8 ***recs,         /* write-only array of nrSteps ptrs to arrays
    ...
    for (c = 0; c < (*nrCols); c++) {
      switch (c) {
        case 0:
          ...
          if (r[0] != (i + 1)) {
            ErrorNested("timestep column (column 1) on line %u is not %u but %g", i + lineDelta + 1,
                        i + 1, r[c]);
            goto error;
          }
          break;
