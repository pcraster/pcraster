...
static void EndError(UINT4 r, UINT4 c, UINT4 nrRows, UINT4 nrCols) {
    ErrorNested(
        "at line '%d' for cell (row= %d, col= %d)\n"
...
static int ScanInputFile(
    ...
    case ASC_ARCINFO: {
        ...
        if (a.nrRows != nrRows || a.nrCols != nrCols) {
            ErrorNested(
                "number of columns and rows are not the same as in the clone map\n"
    ...
    case ASC_GENAMAP: {
        ...
        if (rHeader != nrRows || cHeader != nrCols) {
            ErrorNested(
                "number of rows and columns are not the same as in the clone map\n"
