 ...
 */
static void EndError(UINT4 r, UINT4 c, UINT4 nrRows, UINT4 nrCols)
{
  ErrorNested("at line '%d' for cell (row= %d, col= %d)\n"
              "%d cells were read (%d) required",
              LexGetLineNr(), r + 1, c + 1, (int)((r * nrCols) + c), (int)(nrRows * nrCols));
}

static void PrintIllegalVal(UINT4 r, /* row */
                            UINT4 c) /* col */
{
  ErrorNested("at line '%d' for cell (row=%d,col=%d)", LexGetLineNr(), (int)r + 1, (int)c + 1);
}

/* Parses the input file, checks its values and puts values in csf map.
 * Returns 1 in case of an error, 0 otherwise.
 */
static int ScanInputFile(MAP *out,        /* write-only */
                         FILE *f,         /* input file */
                         const char *mv,  /* missing value */
                         int sepChar,     /* separator char */
                         ASC_TYPE format, /* special format */
                         int header, int rowHeader)
{
  size_t nrMV = 0;
  size_t nrnonMV = 0;
  size_t nrDirFlat = 0;
  ...
  for (r = 0; r < nrRows; r++) {
    ...
    for (c = 0; c < nrCols; c++) {
      ...
      if (token == sepChar) {
        ErrorNested("empty cell, two consecutive separators '%c' read", sepChar);
        EndError(r, c, (UINT4)nrRows, (UINT4)nrCols);
        goto error;
      }
      ...
      switch (token) {
        ...
        default:                         /* not a number, could be the mv */
          if (!number && StrEq(v, mv)) { /* string mv */
            SET_MV_REAL8(buf + c);
            nrMV++;
 ...
 * Returns 1 in case of an error, 0 otherwise
 */
int Asc2Map(MAP *out,                /* write-only output file */
            const char *inputFile,   /* input file */
            const char *mv,          /* missing value */
            int sepChar, ASC_TYPE t, /* special format */
            int header, int rowHeader)
{
  FILE *f = NULL;
