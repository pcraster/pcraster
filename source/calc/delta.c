 ...
 */
static REAL8 GetVal(const MAP_REAL8 *dem, /* Digital Elevation Model */
...
/* See skidmore, 1989 */
void CalcDeltaXY(double *dzDx,         /* write-only, difference in X */
...
#define Z(ir, jc) (z[(ir) + 1][(jc) + 1])
    ...
    *dzDy = ((Z(1, 1) + 2 * Z(1, 0) + Z(1, -1)) - (Z(-1, 1) + 2 * Z(-1, 0) + Z(-1, -1))) / d;
     ...
     */
    *dzDx = ((Z(+1, -1) + 2 * Z(0, -1) + Z(-1, -1)) - (Z(1, 1) + 2 * Z(0, 1) + Z(-1, +1))) / d;
#undef Z
}
 ...
 *     Z7 Z8 Z9
 */
void ZevenbergenGrid(REAL8 *Z, const MAP_REAL8 *dem, int r, int c)
{
