 ...
 */
static int Sum(MAP_REAL8 *state,        /* read-write output state map */
               ...
               ACCU_FUNC f)             /* function to perform */
{
    ...
    if (amount->Get(&amountVal, r, c, amount) && value->Get(&val, r, c, value)) {
 ...
 *         0 otherwise.
 */
static int CalcStateFlux(MAP_REAL8 *state,        /* Read-write output state map  */
                         MAP_REAL8 *flux,         /* Read-write output flux map  */
 ...
 *         0 otherwise.
 */
int PerformAccu(MAP_REAL8 *state,        /* Read-write output state map  */
                MAP_REAL8 *flux,         /* Read-write output flux map  */
                const MAP_UINT1 *ldd,    /* ldd map */
                const MAP_REAL8 *amount, /* amount map */
