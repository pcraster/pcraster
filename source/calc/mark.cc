...
public:
  MarkPoint(const geo::CellLoc &c, double order, double amount)
      : d_c(c), d_order(POSSIBLE_DATA_LOSS(float, order)), d_amount(POSSIBLE_DATA_LOSS(float, amount))
  {
  }
...
public:
  ...
  MarkCondition(double treshold) : d_treshold(treshold)
  {
  }
...
static int MarkWhileSum(MAP_UINT1 *m_resultMap, const MAP_REAL8 *m_order, const MAP_REAL8 *m_amount,
...
extern "C" int MarkWhileSumLe(MAP_UINT1 *m_resultMap, const MAP_REAL8 *m_order,
                              const MAP_REAL8 *m_amount, const MAP_REAL8 *m_treshold)
{
...
extern "C" int MarkUntilSumGe(MAP_UINT1 *m_resultMap, const MAP_REAL8 *m_order,
                              const MAP_REAL8 *m_amount, const MAP_REAL8 *m_treshold)
{
  ReadOnlyReal8_ref(tresholdMap, m_treshold);
  PRECOND(!tresholdMap.spatial());
