 ...
 */
static void SpreadDown(MAP_REAL8 *outCost,        /* write-only output map  */
                       ...
                       int colNr)                 /* point from which is spread */
{
  ...
  while (ldd->Get(&lddVal1, r, c, ldd) && (points->Get(&pointVal1, r, c, points)) &&
    ...
    if (friction->Get(&fricVal2, rNext, cNext, friction) && (cost->Get(&costVal2, rNext, cNext, cost)) &&
        (ldd->Get(&lddVal2, rNext, cNext, ldd)) && (outId->Get(&id2, rNext, cNext, outId)) &&
        (outId->Get(&id1, r, c, outId)) && (outCost->Get(&costVal1, r, c, outCost)) &&
      ...
      if ((id2 == 0) | (outCost->Get(&costVal2, rNext, cNext, outCost) && (totalcost < costVal2))) {
 ...
 * Returns 0.
 */
int SpreadLdd(MAP_REAL8 *outCost,        /* write-only output map  */
              MAP_INT4 *outId,           /* read-write output map  */
              const MAP_UINT1 *ldd,      /* ldd map  */
              const MAP_INT4 *points,    /* points */
              const MAP_REAL8 *cost,     /* initial costs */
