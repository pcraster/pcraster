 ...
 * list
 */
static TT_CELL *NewCell(int r, int c, TT_CELL *list)
{
 ...
 * the new list
 */
static TT_CELL *RemoveCell(TT_CELL *list)
{
  TT_CELL *l = list->prev;
  Free(list);
  return l;
...
static void AddAmount(MAP_REAL8 *m, const TT_CELL *c, double v)
{
  double addVal = NAN;
...
static double timeToCell(const MAP_REAL8 *accumTT, const TT_CELL *c, double startCummTT)
{
  double timeTo = NAN;
...
static int Compute(int rPit,                  /* row of pit cell */
                   ...
                   const MAP_REAL8 *fraction) /* fraction */
{
  ...
  while (list != NULL) {
    ...
    if (list->nextLddDir > 9) { /* this criteria ensures that we compute
       * the cell only once
...
int TravelTime(MAP_REAL8 *state,         /* write-only output map  */
               MAP_REAL8 *flux,          /* write-only output map  */
               MAP_REAL8 *removed,       /* write-only output map  */
               const MAP_UINT1 *ldd,     /* ldd map    */
               const MAP_REAL8 *amount,  /* amount to be transported  */
