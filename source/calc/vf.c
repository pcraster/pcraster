    ...
    for (c = 0; c < ncols; c++) {
      if (scalar->Get(&value, r, c, scalar)) {
        if (scalar->Get(&up, r - 1, c, scalar) && scalar->Get(&down, r + 1, c, scalar)) {
          /* use second order central difference to get the derivative */
          result->Put((up - down) / (2 * dy), r, c, result);
        } else if (scalar->Get(&up, r - 1, c, scalar)) {
          /* use first order finite difference to get the derivative */
          result->Put((up - value) / dy, r, c, result);
        } else if (scalar->Get(&down, r + 1, c, scalar)) {
          /* use first order finite difference to get the derivative */
          result->Put((value - down) / dy, r, c, result);
        } else {
          /* cell is isolated, the derivative is zero */
          result->Put(0, r, c, result);
        }
...
int vf_divergence(MAP_REAL8 *result, const MAP_REAL8 *vectorfieldx, const MAP_REAL8 *vectorfieldy)
{
  int r = 0;
  int c = 0;
  int nrows = 0;
 ...
 * */
int vf_diver(MAP_REAL8 *result, const MAP_REAL8 *vectorfieldx, const MAP_REAL8 *vectorfieldy,
             const MAP_REAL8 *deltax, const MAP_REAL8 *deltay)
{
  int r = 0;
  int c = 0;
  int nrows = 0;
  ...
  for (r = 0; r < nrows; r++) {
    for (c = 0; c < ncols; c++) {
      double res = NAN;
      double value = NAN;
      double valueLeft = NAN;
      double valueRight = NAN;
      double valueUp = NAN;
      double valueDown = NAN;
      double dx = NAN;
      double dy = NAN;
 ...
 * */
int vf_lax(MAP_REAL8 *result, MAP_REAL8 const *input, MAP_REAL8 const *fractMap)
{
  int r = 0;
  int c = 0;
  int nrows = 0;
 ...
 * */
extern int vf_laplacian(MAP_REAL8 *result, const MAP_REAL8 *scalar)
{
  int r = 0;
  int c = 0;
  int nrows = 0;
