...
typedef int (*CALC_CELL)(REAL8 *val,   /* write only */
 ...
 */
static int CmpVal(const DATA *e1, /* pointer to DATA record 1 */
 ...
 * Returns integer according to the result of the comparison.
 */
static int CmpId(const REAL8 **e1, /* pointer to record 1 */
 ...
 */
static int CalcDirection(REAL8 *val, /* write only */
 ...
 * Returns integer according to the result of the comparison.
 */
static int CmpCountVal(const REAL8 **e1, /* pointer to record 1 */
                       const REAL8 **e2) /* pointer to record 2 */
{
...
static int CalcSortTable(REAL8 **recs, size_t nrRecs, size_t idNotUsed)
{
  size_t i = 0;
 ...
 * Returns 1 in case of failed memory allocation 0 otherwise.
 */
static int CalcMajority(REAL8 *val, /* write only */
                        REAL8 **recs, size_t nrRecs)
{
  PRECOND(nrRecs > 1);
 ...
 * Returns 1 in case of failed memory allocation 0 otherwise.
 */
static int CalcMinority(REAL8 *val, /* write only */
                        REAL8 **recs, size_t nrRecs)
{
  PRECOND(nrRecs > 1);
 ...
 * Returns 1 in case of failed memory allocation, 0 otherwise.
 */
static int CalcAverage(REAL8 *val, /* write only */
                       REAL8 **recs, size_t nrRecs)
{
  size_t i = 0;
  double totalVal = 0;
 ...
 * Returns 1 in case of failed memory allocation, 0 otherwise.
 */
static int CalcTotal(REAL8 *val, /* write only */
                     REAL8 **recs, size_t nrRecs)
{
  size_t i = 0;
  double totalVal = 0;
 ...
 * Returns 1 in case of failed memory allocation, 0 otherwise.
 */
static int CalcMax(REAL8 *val, /* write only */
                   REAL8 **recs, size_t nrRecs)
{
  size_t i = 0;
  PRECOND(nrRecs > 0);
...
static void CalcRecordId(const MAP *out) /* file to use for co-ordinate
                                          * calculation
                                          */
{
  size_t i = 0;
 ...
 */
int Col2Map(MAP *out,              /* output file */
            ...
            int sepChar)           /* separator character */
{
  ...
  if (AppReadColumnFile(&orgRecList, &nrRecords, &nrRecordsRead, &nrMVvalueColumn, &nrMVcoordColumn,
                        &geoeas, inputFile, mv, RgetValueScale(out), RgetCellRepr(out), colNr, sepChar,
                        true)) {
  ...
  switch (compCell) {
    case MAJORITY:
      conflictType = "majority";
      break;
    case MINORITY:
      conflictType = "minority";
      break;
    case DIR_AVERAGE:
