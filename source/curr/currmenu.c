...
static bool CurrentVis(const CURR_SELECT_BOX *b) /* read-write radio select box */
{
  ...
  return b->currPosition >= b->firstItemVis && (b->currPosition < (b->firstItemVis + b->nrRulesVis));
}
 ...
 * This is done by modifying the given select box
 */
static void LineUp(CURR_SELECT_BOX *b) /* read-write select box */
{
  if (b->currPosition > 0) {
    b->currPosition--;
    /*
 ...
 * This is done by modifying the given select box.
 */
static void LineDown(CURR_SELECT_BOX *b) /* read-write select box */
{
 ...
 */
CURR_SELECT_BOX *CurrNewItemsInBox(CURR_RADIO_SELECT_BOX *b, /* read-write radio select box */
 ...
 */
static CURR_SELECT_BOX *CurrInitSelectBox(int beginY,         /* start y-coordinate */
 ...
 */
static void CurrFreeSelectBox(CURR_SELECT_BOX *b) /* write and free select box */
{
  Free((void *)b->items);
 ...
 *
 */
CURR_RADIO_SELECT_BOX *CurrInitRadioSelectBox(int beginY,           /* start y-coordinate */
                                              int beginX,           /* start x-coordinate */
                                              ...
                                              const char *otherMsg) /* other message */
{
  ...
  radioBox =
      CurrInitSelectBox(beginY, beginX, nrRulesVis, nrCols, items, nrItems, otherKeys, nrKeys, otherMsg);
  return radioBox;
}
 ...
 *
 */
CURR_MULTI_SELECT_BOX *CurrInitMultiSelectBox(int beginY,           /* start y-coordinate */
                                              int beginX,           /* start x-coordinate */
                                              int nrRulesVis,       /* number of rules in the window */
                                              int nrCols,           /* number of cols in the window */
                                              const char **items,   /* items to select */
                                              ...
                                              int nrKeys)           /* number of other keys */
{
  ...
  b = CurrInitSelectBox(beginY, beginX, nrRulesVis, nrCols, items, nrItems, otherKeys, nrKeys, NULL);
  if (b == NULL)
    return NULL;
...
void CurrRadioPrintItems(CURR_SELECT_BOX *b) /* box to print items from */
{
                           ...
                           CURR_SELECT_BOX *b) /* read-write radio select box */
{
  ...
  const KEY_2_COM keys[] = {
      {KEY_NPAGE, (KEY_FUNC)PageDown}, {KEY_PPAGE, (KEY_FUNC)PageUp},  {'j', (KEY_FUNC)LineDown},
      {KEY_SR, (KEY_FUNC)LineDown},    {KEY_DOWN, (KEY_FUNC)LineDown}, {'k', (KEY_FUNC)LineUp},
      {KEY_SF, (KEY_FUNC)LineUp},      {KEY_UP, (KEY_FUNC)LineUp}
      /*
         *  { 'D' , (KEY_FUNC)PageDown},
         *  { 'U',  (KEY_FUNC)PageUp}
         */
...
{
  b->highlighted[b->box->currPosition] = !b->highlighted[b->box->currPosition];
}
...
static void DelChar(char *s, /* read-write string to modify */
 ...
 */
static void InsertChar(char *s, /* read-write string to modify */
  ...
  while (1) {
    ...
    switch (c) {
      ...
      case CURR_KEY_ESC:
        return FALSE;
      default:
        if (IsPrint(c) && c != KEY_IC && c != KEY_INSERT && col < (len - 1)) {
