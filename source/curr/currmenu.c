 ...
 */
static CURR_SELECT_BOX *
CurrInitSelectBox(int beginY,           /* start y-coordinate */
                       ...
                       const char *otherMsg) /* other message */
{
    ...
    radioBox = CurrInitSelectBox(
        beginY, beginX, nrRulesVis, nrCols, items, nrItems, otherKeys, nrKeys, otherMsg);
    return radioBox;
}
                       ...
                       int nrKeys)           /* number of other keys */
{
    ...
    b = CurrInitSelectBox(
        beginY, beginX, nrRulesVis, nrCols, items, nrItems, otherKeys, nrKeys, NULL);
    if (b == NULL)
        return NULL;
...
#ifndef DJGPP
    Curr_wprintw(msgBox,
                 " ArrowLeft=GoOneLeft; ArrowRight=GoOneRight; Del,BackSpace=DeleteOneLeft\n");
#else
