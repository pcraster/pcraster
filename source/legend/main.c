...
static MAP *OpenClassMap(const char *name, enum MOPEN_PERM perm)
{
  ...
  if (!AppIsClassified(vs) || !RvalueScale2(vs) || vs == VS_LDD) {
    Error("map '%s' has '%s' data type, no legend possible", name, RstrValueScale(vs));
    Mclose(in);
    return NULL;
  }
  RuseAs(in, CR_INT4);
...
static CSF_LEGEND *ReadMapLegends(int *nrOut, char **mapNames, int nrMapNames,
...
{
  ErrorNested("line '%d' only '%d' characters allowed for legend entry", line,
 ...
 */
static CSF_LEGEND *ReadLegendFile(int *nrOut, const char *inputFileName, CSF_VS vs, CSF_CR cr)
{
  ...
  while (memset(buf, '\0', LINE_LENGTH) != NULL && (fgets(buf, (int)LINE_LENGTH, f) != NULL)) {
    INT4 nr = 0;
    char *descr = NULL;
    char *nrPtr = NULL;
         ...
         char *argv[]) /* list of arguments */
{
  ...
  for (i = 1; i < nrMaps; i++) {
    ...
    if (vs != valueScale) {
      Error("Conflicting data types: '%s' is %s,'%s' is %s", argv[1], RstrValueScale(valueScale),
            argv[i + 1], RstrValueScale(vs));
      Mclose(m);
      goto failure;
    }
