...
static int PrintHeader(const char **mapNames,
                       int nrMapNames,
...
static int PrintLegend(int startY, CSF_LEGEND *leg, int nrLeg)
{
    ...
    legBox = CurrInitRadioSelectBox(startY,
                                    colXStart,
                                    LINES - startY - 4,
                                    boxCols,
                                    (const char **)legItems,
    ...
    while (i == 0) {
        if (CurrRadioSelectItem(&i, legBox)) {
            ...
            if (CurrGetString(leg[i].descr,
                              CSF_LEGEND_DESCR_SIZE - 1,
                              startY + i - legBox->firstItemVis,
                              colXStart + prefLen)) {
                lastEdit = i;
...
extern int Menu(CSF_LEGEND *leg, int nrLeg, const char **mapNames, int nrMapNames)
{
    ...
    headerLines = PrintHeader(
        mapNames, nrMapNames, nrLeg == 1 ? MV_INT4 : leg[1].nr, leg[nrLeg - 1].nr);
