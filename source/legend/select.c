...
static int PrintHeader(const char **mapNames, int nrMapNames, INT4 l, /* low value, MV_INT4 if all mvs */
...
static void WriteItems(const CSF_LEGEND *leg, int nrLeg, int numLen)
{
  int i = 0;
...
static int PrintLegend(int startY, CSF_LEGEND *leg, int nrLeg)
{
  ...
  while (i == 0) {
    if (CurrRadioSelectItem(&i, legBox)) {
      ...
      if (CurrGetString(leg[i].descr, CSF_LEGEND_DESCR_SIZE - 1, startY + i - legBox->firstItemVis,
                        colXStart + prefLen)) {
        lastEdit = i;
      ...
      switch (i) { /* it's an otherKey */
        case 'q':
          if (lastEdit != -1) {
            ...
            switch (CurrPromptYesNo("Write modified legend to maps?", "ResumeEditing")) {
              case 'y':
                return 1;
              case 'n':
                return 2;
