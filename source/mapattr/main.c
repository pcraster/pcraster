 ...
 */
static char const *printLabels[] = {"rows       ", "columns    ", "data_type  ", "cell_repr  ",
                                    "projection ", "xUL        ", "yUL        ", "cell_length",
                                    "angle(deg) ", "file_id    ", "min_val    ", "max_val    ",
                                    "version    ", "native     ", "attr_tab   "};
...
static int PrintOption(const char **names, int nrNames)
{
  ...
  for (i = 0; i < nrNames; i++) {
    ...
    switch (a[i].cellRepr) {
      ...
      case CR_REAL8:
        ...
        break;
      default:
        cStr = RstrCellRepr(a[i].cellRepr);
    }
...
static int CloneOption(const char *name)
{
  ATTRIBUTES a;
  if (FileStat(name) != 2) {
...
static int EditOption(const char *name)
{
  ATTRIBUTES a;
                               ...
                               const char **names, int nrNames)
{
  ...
  for (i = 0; i < nrNames; i++) {
    ...
    if (RgetNrRows(maps[i]) != a->nrRows || RgetNrCols(maps[i]) != a->nrCols) {
      Error("number of rows and columns of '%s' do not match '%s'", names[i], names[0]);
      goto error;
    }
  }
  return maps;
    ...
    switch (c) {
      ...
      case 'R':
        if (*((const int *)OptArg) <= 0) {
          Error("-R number of rows must be greater than 0 (not '%d)", *((const int *)OptArg));
          goto failure;
        }
      ...
      case 'C':
        if (*((const int *)OptArg) <= 0) {
          Error("-R number of columns must be greater than 0 (not '%d)", *((const int *)OptArg));
          goto failure;
        }
      ...
      case 'i':
        if (*((const int *)OptArg) < 0) {
          Error("-i file id must be >= 0 (not '%d)", *((const int *)OptArg));
          goto failure;
        }
      ...
      case 'P':
        ...
        goto failure;
      case 'x':
        opt.xUL = *((const double *)OptArg);
        break;
      case 'y':
        opt.yUL = *((const double *)OptArg);
        break;
      case 'l':
        if (*((const double *)OptArg) <= 0) {
          Error("-l cell length must be greater than 0 (not '%g)", *((const double *)OptArg));
          goto failure;
        }
        opt.cellSize = *((const double *)OptArg);
        break;
      case 'a':
        if ((*((const double *)OptArg) < -90) || (*((const double *)OptArg) > 90)) {
          Error("-a angle must be between -90 and 90 (not '%g)", *((const double *)OptArg));
          goto failure;
        }
        opt.angle = *((const double *)OptArg);
        break;
#include "case_vs.h"
    } /* eoswitch */
  } /* eowhile */
