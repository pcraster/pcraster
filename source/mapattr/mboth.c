...
const char *AttrStr(const ATTRIBUTES *a, ATTR_NRS i)
{
  ...
  switch (i) {
    ...
    case ATTR_valueScale:
      sprintf(buf, "%s", RstrValueScale(a->valueScale));
      break;
    case ATTR_cellRepr:
      sprintf(buf, "%s", RstrCellRepr(a->cellRepr));
      break;
    case ATTR_projection:
      sprintf(buf, "%s", MstrProjection(a->projection));
      break;
    case ATTR_xUL:
      PrintFloat(buf, a->xUL);
      break;
    case ATTR_yUL:
      PrintFloat(buf, a->yUL);
      break;
    case ATTR_cellSize:
      PrintFloat(buf, a->cellSize);
      break;
    case ATTR_angle:
      PrintFloat(buf, a->angle);
      break;
    case ATTR_gisFileId:
...
void GetAttrDouble(double *v, const ATTRIBUTES *a, ATTR_NRS i)
{
  switch (i) {
    case ATTR_nrRows:
      *v = a->nrRows;
...
void SetAttrDouble(ATTRIBUTES *a, const double *v, ATTR_NRS i)
{
  switch (i) {
    case ATTR_nrRows:
      a->nrRows = (UINT4)*v;
    ...
    case ATTR_nrCols:
      ...
      break;
    case ATTR_valueScale:
      a->valueScale = (CSF_VS)*v;
      break;
    case ATTR_cellRepr:
      a->cellRepr = (CSF_CR)*v;
      break;
    case ATTR_projection:
      a->projection = (CSF_PT)*v;
      break;
    case ATTR_xUL:
      a->xUL = *v;
      break;
...
static int EditProjection(double *editValue, /* read-write */
...
bool EditItem(double *editValue,            /* read-write */
              char *editString, ATTR_NRS i, /* item */
              int yStart, int xStart)
{
  switch (i) {
    case ATTR_nrRows:
