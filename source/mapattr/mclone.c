...
static int PrintHeader(const char *mapName)
...
static int PrintLegend(int startY, ATTRIBUTES *attr, int nrAttr)
{
    ...
    attrBox = CurrInitRadioSelectBox(startY,
                                     colXStart,
                                     LINES - startY - 4,
                                     boxCols,
                                     (const char **)attrItems,
                                     nrAttr,
                                     otherKeys,
    ...
    while (i == 0) {
        if (CurrRadioSelectItem(&i, attrBox)) {
            ...
            if (EditItem(&edit,
                         strcpy(editBuf, AttrStr(attr, i)),
                         i,
                         startY + i - attrBox->firstItemVis,
                         colXStart + prefLen)) {
                lastEdit = i;
                lastEditOrig = orig;
            ...
            case 'u':
                if (lastEdit != -1) {
                    ...
                    WriteItems(attrItems, attr, nrAttr);
                    if (CurrNewItemsInBox(attrBox, (const char **)attrItems, nrAttr) ==
                        NULL)
                        return 0;
                }
                break;
