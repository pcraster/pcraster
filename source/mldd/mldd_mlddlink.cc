...
static REAL4 *inCast(const calc::ModelLinkArgSpec &a)
{
  ...
  return (REAL4 *)(static_cast<const MAP_REAL8 *>(a.value)->spatialValue[0]);
}
...
static UINT1 *inCast1(const calc::ModelLinkArgSpec &a)
{
  ...
  PRECOND(a.st == PCR_ST_SPATIAL);
  return (UINT1 *)(static_cast<const MAP_UINT1 *>(a.value)->spatialValue[0]);
}
...
static REAL4 *outCast(const calc::ModelLinkArgSpec &a)
{
  PRECOND(a.cr == PCR_CR_REAL8);
  PRECOND(a.st == PCR_ST_SPATIAL);
  return (REAL4 *)(static_cast<MAP_REAL8 *>(a.value)->spatialValue[0]);
}
...
static UINT1 *outCast1(const calc::ModelLinkArgSpec &a)
{
  PRECOND(a.cr == PCR_CR_UINT1);
  PRECOND(a.st == PCR_ST_SPATIAL);
  return (UINT1 *)(static_cast<MAP_UINT1 *>(a.value)->spatialValue[0]);
}
...
mldd::MlddLink::MlddLink() : calc::ModelLink(), d_mldd(0)
{
}
...
bool mldd::MlddLink::methodCheck(const std::string &methodName, Sig &sig) const
{
...
void mldd::MlddLink::methodExecute(const std::string &methodName, Sig &sig)
{
...
void mldd::MlddLink::initExecute(const geo::RasterSpace &rs, Sig &sig)
{
...
void mldd::MlddLink::upstreamExecute(Sig &sig) const
{
  ...
  d_mldd->upstream(outCast(sig.d_result[0]), inCast(sig.d_input[0]));
}
...
void mldd::MlddLink::diffuseExecute(Sig &sig) const
{
  ...
  INT4 nrIterations = (static_cast<const MAP_INT4 *>(sig.d_input[11].value))->nonSpatialValue;
  ...
  d_mldd->diffuse(outCast(sig.d_result[0]),
                  ...
                  diffusionValueInArgOrder, nrIterations);
}
