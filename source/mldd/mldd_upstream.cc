...
mldd::Upstream::Upstream(const WeightMap &w, const REAL4 *in, REAL4 *result)
    : DownstreamVisitor(w.rasterDim()), d_w(w), d_in(in), d_result(result)
{
...
void mldd::Upstream::initVertex(const Vertex &v)
{
  d_result[linear(v)] = 0;
}
