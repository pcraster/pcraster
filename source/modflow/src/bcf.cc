...
void BCF::writeBCF()
{
  ...
  size_t const count = d_mf->d_layer2BlockLayer.size();
  ...
  for (int i = count - 1; i >= 0; i--) {
    size_t const mfLayer = count - i;
    size_t const size = d_mf->d_layer2BlockLayer.size();
    size_t const blockLayer = d_mf->d_layer2BlockLayer.at(size - mfLayer);
  ...
  for (int i = count - 1; i >= 0; i--) {
    ...
    size_t const lcon = getLaycon(d_mf->d_layerType.at(blockLayer));
    //
    ...
    if (d_mf->d_isSteadyState == false) {
      ...
      d_mf->d_cmethods->writeMatrix(content, stmp.str(), d_mf->d_layer2BlockLayer,
                                    *(d_mf->d_primaryStorage), blockLayer);
    }
    //
    ...
    if ((lcon == 1) || (lcon == 3)) {
      std::string const s = std::format("INTERNAL  1.00000E+00  (FREE)  -1    HY layer {}", mfLayer);
    ...
    if ((i != 0) && (blockLayer != 0)) {  // not for bottom layer, check this again...
      std::string const s = std::format("INTERNAL  1.00000E+00  (FREE)  -1    VCONT layer {}", mfLayer);
    ...
    if (d_mf->d_isSteadyState == false) {
      if ((lcon == 2) || (lcon == 3)) {
        ...
        d_mf->d_cmethods->writeMatrix(content, stmp.str(), d_mf->d_layer2BlockLayer,
                                      *(d_mf->d_secondaryStorage), blockLayer);
      }
    }
    //
    ...
    if ((d_iwdflg < 0.0) || (d_iwdflg > 0.0)) {
      ...
      if ((lcon == 1) || (lcon == 3)) {
        ...
        d_mf->d_cmethods->writeMatrix(content, stmp.str(), d_mf->d_layer2BlockLayer, *(d_mf->d_wetting),
                                      blockLayer);
      }
    }
  }
...
{
  size_t const cols = d_mf->d_nrOfColumns;
  size_t const rowWrap = d_mf->d_nrOfColumns - 1;
  aStream << msg << "\n";

  for (size_t j = 0; j < d_mf->d_nrOfCells; ++j) {
    // tran = height * hcond
    float const tran = d_mf->d_baseArea->cell(j)[layer] * d_mf->d_hCond->cell(j)[layer];
    aStream << " " << tran;
    if ((j % cols) == rowWrap) {
      aStream << "\n";
    }
...
void BCF::calcVCond(std::stringstream &aStream, size_t layer, const std::string &msg)
{
  ...
  size_t const rowWrap = d_mf->d_nrOfColumns - 1;
  aStream << msg << "\n";
  bool const res = hasConfinedSubLayer(layer);
                  ...
                  bool calc)
{
  ...
  bool const result = d_mf->setBlockData(*(d_mf->d_hCond), hcond->src_f(), layer);
  if (result == true) {
...
void BCF::get_binary(float *values, const std::string &description, size_t start, size_t multiplier,
                     std::string const &path) const
{
  ...
  size_t const nr_cells = d_mf->d_nrOfCells;
  int const nr_result_layer = d_mf->nr_modflow_layer();
...
calc::Field *BCF::get_storage(size_t layer, std::string const &path) const
{
  ...
  size_t const start_pos = 0;  // 'first' entry in transient simulations
...
void BCF::write_vcond(std::string const &path)
{
  ...
  for (int i = count - 1; i >= 0; i--) {
    ...
    if ((i != 0) && (blockLayer != 0)) {
      ...
      size_t const cols = d_mf->d_nrOfColumns;
      size_t const rowWrap = d_mf->d_nrOfColumns - 1;
      bool const res = hasConfinedSubLayer(blockLayer);
