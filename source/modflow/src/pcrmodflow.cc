...
{
  if (nullptr == d_rch) {
    d_cmethods->error("No recharge package specified ", "getRecharge");
  }
...
{
  if (nullptr == d_riv) {
    d_cmethods->error("No river package specified: Define river head, bottom and conductance values ", "getRiverLeakage");
  }
...
bool PCRModflow::writeNAM()
{
  ...
  //
  content << "LIST  206 pcrmf_lst.txt\n";  
  content << "DATA  232 " << d_bas->output_ibound_filename() << " REPLACE\n";  
  content << "# Input files\n";
  //
  content << "DIS   208 " << d_dis->filename() << "\n";
  content << "DATA  300 " << d_dis->data_filename() << "\n";
  //
  content << "BAS6  207 " << d_bas->filename() << "\n";
  content << "DATA  400 " << d_bas->data_heads_filename() << "\n";
  content << "DATA  401 " << d_bas->data_ibound_filename() << "\n";

  content << "BCF6  209 " << d_bcf->filename() << "\n";
  content << "DATA  " << d_bcf->hy_unit_number() << " " << d_bcf->data_hy_filename() << "\n";
  content << "DATA  " << d_bcf->vcond_unit_number() << " " << d_bcf->data_vcond_filename() << "\n";
  content << "DATA  " << d_bcf->tran_unit_number() << " " << d_bcf->data_tran_filename() << "\n";

  if (d_bcf->transient()) {
    content << "DATA  " << d_bcf->sf1_unit_number() << " " << d_bcf->data_sf1_filename() << "\n";
    content << "DATA  " << d_bcf->sf2_unit_number() << " " << d_bcf->data_sf2_filename() << "\n";
  }

  if (d_bcf->rewetting()) {
    content << "DATA  " << d_bcf->wet_unit_number() << " " << d_bcf->data_rewet_filename() << "\n";
  }


  if (d_solver_used == PCG_SOLVER) {
    content << "PCG   210 " << d_pcg->filename() << "\n";
  }
  if (d_solver_used == SIP_SOLVER) {
    content << "SIP   210 " << d_sip->filename() << "\n";
  }
  if (d_solver_used == SOR_SOLVER) {
    content << "SOR   210 " << d_sor->filename() << "\n";
  }
  if (d_solver_used == DSP_SOLVER) {
    content << "DE4   210 " << d_dsp->filename() << "\n";
  }
  content << "OC    220 pcrmf_oc.txt\n";
  if (d_riv != nullptr) {
    content << "RIV   221 " << d_riv->filename() << "\n";
    content << "DATA  251 " << d_riv->data_filename() << "\n";
  }

  if (d_rch != nullptr) {
    content << "RCH   222 " << d_rch->filename() << "\n";
    content << "DATA  261 " << d_rch->data_rch_filename() << "\n";
    if (d_rch->indicated_recharge()) {
      content << "DATA  262 " << d_rch->data_irch_filename() << "\n";
    }
  }

  if (d_drn != nullptr) {
    content << "DRN   223 " << d_drn->filename() << "\n";
    content << "DATA  270 " << d_drn->data_filename() << "\n";
  }

  if (d_wel != nullptr) {
    content << "WEL   224 " << d_wel->filename() << "\n";
    content << "DATA  280 " << d_wel->data_filename() << "\n";
  }

  if (d_ghb != nullptr) {
    content << "GHB   225 " << d_ghb->filename() << "\n";
    content << "DATA  256 " << d_ghb->data_filename() << "\n";
  }
  
  
  content << "# Binary output files\n";
  content << "DATA(BINARY)  231 " << d_bas->output_heads_filename() << " REPLACE\n";  
  content << "DATA(BINARY)  240 " << d_bcf->output_bcf_filename() << " REPLACE\n";
  
  if (d_riv != nullptr) {
    content << "DATA(BINARY)  250 " << d_riv->output_riv_filename() << " REPLACE\n";
  }
  if (d_rch != nullptr) {
    content << "DATA(BINARY)  260 " << d_rch->output_rch_filename() << " REPLACE\n";
  }
  if (d_drn != nullptr) {
    content << "DATA(BINARY)  271 " << d_drn->output_drain_filename() << " REPLACE\n";
  }  
  if (d_ghb != nullptr) {
    content << "DATA(BINARY)  272 " << d_ghb->output_ghb_filename() << " REPLACE\n";    
  }
  
  return d_cmethods->writeToFile(mf::execution_path(run_directory(), "pcrmf_nam.txt"), content.str());
}
...
bool PCRModflow::writeOC()
{
  std::string content = std::format(
    ...
    "SAVE IBOUND\n"
    "SAVE BUDGET\n",
    d_bas->fortran_unit_number_heads(), d_bas->fortran_unit_number_bounds(), d_dis->getTimeSteps());
  return d_cmethods->writeToFile(mf::execution_path(run_directory(), "pcrmf_oc.txt"), content);
}
...
bool PCRModflow::setIBound(const int *values, size_t layer)
{
  ...
  bool const result = setBlockData(*d_ibound, values, layer);
  return result;
}
...
bool PCRModflow::setInitialHead(const float *values, size_t layer)
{
  ...
  //size_t blockLayer = mfLayer2BlockLayer(mfLayer);
  bool const result = setBlockData(*d_initialHead, values, layer);
  return result;
}
...
bool PCRModflow::setPrimaryStorage(const float *values, size_t layer)
{
  ...
  bool const result = setBlockData(*d_primaryStorage, values, layer);
  return result;
}
...
bool PCRModflow::setSecondaryStorage(const float *values, size_t layer)
{
  ...
  bool const result = setBlockData(*d_secondaryStorage, values, layer);
  return result;
}
...
bool PCRModflow::setWetting(const float *values, size_t layer)
{
  ...
  bool const result = setBlockData(*d_wetting, values, layer);
  return result;
}
...
bool PCRModflow::runModflow(const std::filesystem::path &working_directory)
{
  ...
  if (d_solver_used == NO_SOLVER) {
    std::string const stmp("Can not execute Modflow: No solver package specified");
    d_cmethods->error(stmp, "run");
  }
  ...
  // old listing file
  removeTextFiles(mf::execution_path(run_directory(), "pcrmf_lst.txt"));
  ...
  if (d_pcg != nullptr) {
    if (d_pcg->modified()) {
      std::stringstream content;
      content << *d_pcg;
      d_cmethods->writeToFile(mf::execution_path(run_directory(), d_pcg->filename()), content.str());
      d_pcg->update();
    }
  }
  if (d_sip != nullptr) {
    if (d_sip->modified()) {
      std::stringstream content;
      content << *d_sip;
      d_cmethods->writeToFile(mf::execution_path(run_directory(), d_sip->filename()), content.str());
      d_sip->update();
    }
  }
  if (d_sor != nullptr) {
    if (d_sor->modified()) {
      std::stringstream content;
      content << *d_sor;
      d_cmethods->writeToFile(mf::execution_path(run_directory(), d_sor->filename()), content.str());
      d_sor->update();
    }
  }
  if (d_dsp != nullptr) {
    if (d_dsp->modified()) {
      std::stringstream content;
      content << *d_dsp;
      d_cmethods->writeToFile(mf::execution_path(run_directory(), d_dsp->filename()), content.str());
      d_dsp->update();
    }
  }
  ///==============================================
...
void PCRModflow::modflow_converged()
{
  ...
  std::string const filename = mf::execution_path(run_directory(), "pcrmf_lst.txt");
  ...
  std::string const search("TO MEET SOLVER CONVERGENCE CRITERIA");
...
{
  if (d_ghb == nullptr) {
    d_cmethods->error("No general head boundary package specified: Define head and conductance values ", "getGeneralHeadLeakage");
  }
