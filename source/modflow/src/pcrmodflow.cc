...
void PCRModflow::getRecharge(float *result, size_t mfLayer){
  ...
  d_rch->getRecharge(result, mfLayer);
}
...
calc::Field* PCRModflow::getRecharge(size_t layer){
  ...
  return d_rch->getRecharge(layer);
}
...
void PCRModflow::getRiverLeakage(float *result, size_t layer){
  ...
  d_riv->getRiverLeakage(result, layer);
}
...
calc::Field* PCRModflow::getRiverLeakage(size_t layer){
  ...
  return d_riv->getRiverLeakage(layer);
}
...
bool PCRModflow::writeNAM() {
  ...
  content << "# Output files" << std::endl;
  content << "LIST  206 pcrmf.lst" << std::endl;
  content << "# Input files" << std::endl;
...
bool PCRModflow::writeOC() {
  std::stringstream content;
  content << "# Generated by PCRaster Modflow" << std::endl;
   content << "HEAD SAVE UNIT " << d_bas->fortran_unit_number_heads() << std::endl;
  content << "IBOUND SAVE UNIT " << d_bas->fortran_unit_number_bounds() << std::endl;
  content << "PERIOD 1 STEP " << d_dis->getTimeSteps() << std::endl;
  content << "SAVE HEAD" << std::endl;
  content << "SAVE IBOUND" << std::endl;
  content << "SAVE BUDGET" << std::endl;
  return d_cmethods->writeToFile("pcrmf.oc", content.str());
}
