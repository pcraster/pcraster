...
AguilaGuiTest::AguilaGuiTest()
    ...
    :  // Cursor Window
      ...
      //     Legend view
      //     Plot view
      d_nrVisualisationsPerTimePlotWindow(4), d_nrVisualisationsPerAnimationDialog(1)
...
void AguilaGuiTest::testNotExisting(std::string const &name)
{
  int argc = 2;
  ...
  try {
    ...
    QCOMPARE(exception.message(), std::string("Data source " + name + ":\ncannot be opened"));
  } catch (...) {
  }
...
{
  int argc = 2;
  char const *argv[2] = {"aguila", "MinMaxEqual.map"};
...
void AguilaGuiTest::testDataset1()
{
  {
    int argc = 2;
    char const *argv[2] = {"aguila", "dataset1/aap/scalar_10"};
  ...
  {
    int argc = 4;
    char const *argv[4] = {"aguila", "--scenarios", "{dataset1/aap, dataset1/noot, dataset1/mies}",
                           "scalar_10"};
  ...
  {
    int argc = 6;
    char const *argv[6] = {"aguila",      "--scenarios", "{dataset1/aap, dataset1/noot, dataset1/mies}",
                           "--timesteps", "[1, 30]",     "scalar"};
...

  {
    int argc = 8;
    char const *argv[8] = {
        "aguila",      "--multi", "2x2",   "--scenarios", "{dataset1/aap, dataset1/noot, dataset1/mies}",
        "--timesteps", "[1, 30]", "scalar"};
