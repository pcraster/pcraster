...
CursorView::CursorView(DataObject *object, QWidget *parent)
...
void CursorView::updateEditLabel(dal::Dimension const *dimension, dal::DataSpaceAddress const &address)
{
  ...
  if (dimension->meaning() == dal::Space) {
    value->setText(
        QString::fromUtf8(dal::coordinateToString(space, address, space.indexOf(dimension)).c_str()));
  } else {
    value->setText(
        QString(dataObject().globalToWorldMapper().toString(address, space.indexOf(dimension)).c_str()));
  }
}

dal::DataSpaceAddress CursorView::addressWithUpdatedCoordinate(dal::Dimension const *dimension,
                                                               size_t index) const
{
  assert(dimension->meaning() != dal::Scenarios && dimension->meaning() != dal::Space);
  ...
  switch (dimension->meaning()) {
    ...
    case dal::CumulativeProbabilities: {
      address.setCoordinate<float>(dimensionIndex, dimension->coordinate<float>(index));
      break;
    }
    case dal::Samples:
    case dal::Time: {
      /// FEATURE remove
      /// case dal::Space: {
      address.setCoordinate<size_t>(dimensionIndex, dimension->coordinate<size_t>(index));
      break;
    }
    default: {
      assert(false);
...
void CursorView::updateCoordinate(dal::Dimension const *dimension, size_t index)
{
  assert(dimension->meaning() != dal::Scenarios && dimension->meaning() != dal::Space);

  dataObject().setDataSpaceAddress(addressWithUpdatedCoordinate(dimension, index));
}

size_t CursorView::tupleIndex(dal::Dimension const *dimension) const
{
...
DimensionCoordinateEdit *CursorView::dimensionCoordinateEdit(size_t index)
{
  return std::get<1>(d_dimensionTuples[index]);
}

QLabel *CursorView::valueLabel(size_t index)
{
...
void CursorView::createInterface()
{
  ...
  for (size_t i = 0; i < space.size(); ++i) {
    ...
    if(dimension.meaning() != dal::Scenarios /* &&
      ...
      title = new QLabel(QString(dal::dimensionToString(dimension).c_str()), dimensionEditGroupBox);
      gridLayout->addWidget(title, i, 0);

      if (dimension.meaning() == dal::Space) {
        edit = nullptr;
        value = new QLabel(QString(dal::coordinateToString(space, address, i).c_str()), this);
...
void CursorView::addAttribute(DataGuide const &guide)
{
