...
*/
template <class T>
DataManager<T>::DataManager(geo::DataType dataType)
...
/*!
*/
template <class T> DataManager<T>::~DataManager()
{
...
*/
template <class T> bool DataManager<T>::isConsistent() const
{
  bool result = true;
...
/*!
  ...
  \sa        isConsistent()
*/
template <class T> bool DataManager<T>::isConsistent(DataGuide const &guide) const
{
...
typename DataManager<T>::guide_iter DataManager<T>::findGuide(DataGuide::Address const &address)
{
  return std::find_if(_guides.begin(), _guides.end(),
                      [capture0 = static_cast<T const *>(address)](auto &&PH1) {
                        return addressOfDataGuideIs<T>()(std::forward<decltype(PH1)>(PH1), capture0);
                      });
}
...
DataManager<T>::findData(DataGuide::Address const &address) const
{
  return std::find_if(_data.begin(), _data.end(),
                      [capture0 = static_cast<T const *>(address)](auto &&PH1) {
                        return addressOfDataInfoIs<T>()(std::forward<decltype(PH1)>(PH1), capture0);
                      });
}
...
*/
template <class T> bool DataManager<T>::exists(DataGuide::Address const &address) const
{
...
*/
template <class T> bool DataManager<T>::exists(DataGuide const &guide) const
{
  return dev::hasElement(_guides, guide);
}

template <class T> DataGuide DataManager<T>::add(DataInfo<T> const &info)
{
  assert(isConsistent());

  // Let's see if we already know about the data in info.
  auto const it = std::find(_data.begin(), _data.end(), info);
  ...
  auto const guide_it = findGuide(info.data());
  ...
  if (guide_it != _guides.end()) {
    ...
    result = DataGuide(_guides.size(), static_cast<DataGuide::Address>(info.data()), _dataType,
                       info.valueScale());
...
*/
template <class T> void DataManager<T>::remove(DataGuide const &guide)
{
  assert(isConsistent());
  ...
  if (guide_it == _guides.end()) {
    ...
    auto const data_it = findData(copyOfGuide.address());
...
template <class T> T &DataManager<T>::data(DataGuide const &guide)
{
  assert(exists(guide));
  assert(exists(guide.address()));

  auto const it = findData(guide.address());
  assert(it != _data.end());
...
/*!
  ...
  \return    Data.
*/
template <class T> const T &DataManager<T>::data(DataGuide const &guide) const
{
  assert(exists(guide));
  assert(exists(guide.address()));
...
*/
template <class T> size_t DataManager<T>::size() const
{
  return _data.size();
}

template <class T> bool DataManager<T>::empty() const
{
  return _data.empty();
}
...
*/
template <class T> bool DataManager<T>::isValid(DataGuide const &guide) const
{
...
template <class T> std::set<size_t> DataManager<T>::timeSteps(DataGuide const &guide) const
{
  assert(exists(guide));
  assert(exists(guide.address()));
...
template <class T> std::set<size_t> DataManager<T>::timeSteps() const
{
  ...
  for (DataInfo<T> const &info : _data) {
    ...
    std::set_union(steps.begin(), steps.end(), tmpSteps.begin(), tmpSteps.end(),
...
template <class T> const DataGuide &DataManager<T>::dataGuide(geo::DataGuide const &guide) const
{
