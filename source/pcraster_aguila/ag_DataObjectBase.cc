...
*/
template <class T>
DataObjectBase<T>::DataObjectBase(geo::DataType dataType)
...
*/
template <class T> DataObjectBase<T>::~DataObjectBase()
{
  clear();
}
                                                               ...
                                                               dal::DataSpace const &space) const
{
  ...
  while (result != _tuples.end()) {
    ...
    if (std::get<0>(*result) == name && std::get<1>(*result) == space) {
      break;
    }
...
template <class T>
bool DataObjectBase<T>::exists(std::string const &name, dal::DataSpace const &space) const
{
  return find(name, space) != _tuples.end();
}

template <class T> DataGuide DataObjectBase<T>::add(std::string const &name, dal::DataSpace const &space)
{
  assert(space.hasScenarios() ? space.dimension(space.indexOf(dal::Scenarios)).nrValues() == 1 : true);
...
template <class T> void DataObjectBase<T>::clear()
{
...
*/
template <class T> void DataObjectBase<T>::remove(DataGuide const &guide)
{
...
template <class T> size_t DataObjectBase<T>::size() const
{
  return _manager.size();
}

template <class T> bool DataObjectBase<T>::empty() const
{
  return _manager.empty();
}

template <class T> bool DataObjectBase<T>::isValid(DataGuide const &guide) const
{
  return _manager.isValid(guide);
}

template <class T> std::string DataObjectBase<T>::name(DataGuide const &guide) const
{
  assert(isValid(guide));
...
template <class T> T &DataObjectBase<T>::data(DataGuide const &guide)
{
  return _manager.data(guide);
}

template <class T> const T &DataObjectBase<T>::data(DataGuide const &guide) const
{
  return _manager.data(guide);
}
...
template <class T> DataGuide const &DataObjectBase<T>::dataGuide(geo::DataGuide const &guide) const
{
...
template <class T>
void DataObjectBase<T>::read(dal::DataSpace const &space, dal::DataSpaceAddress const &address)
{
...
template <class T>
bool DataObjectBase<T>::isRead(dal::DataSpace const &space, dal::DataSpaceAddress const &address) const
{
  for (auto it = this->data_begin(); it != this->data_end(); ++it) {
