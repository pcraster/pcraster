...
ag::DrawPropertiesWidget::DrawPropertiesWidget(DataObject &dataObject, const DataGuide &dataGuide,
                                               QWidget *parent)
...
void ag::DrawPropertiesWidget::createPaletteInterface()
{
  ...
  d_data->d_paletteBar->setToolTip("Click on palette for list of possible palettes");
...
void ag::DrawPropertiesWidget::configurePaletteInterface()
{
  ...
  d_data->d_paletteBar->setPalette(dataObject().properties().palette(dataGuide()));

  // Set possible palettes to choose from.
  addPalettes(dataObject().properties().palettes(dataGuide().valueScale()));
}

void ag::DrawPropertiesWidget::addPalettes(const std::vector<const com::RawPalette *> &palettes)
{
...
{
  if (d_data->d_paletteBar->palette() != dataObject().properties().palette(dataGuide())) {
