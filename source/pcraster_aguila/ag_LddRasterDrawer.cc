...
LddRasterDrawer::LddRasterDrawer(Raster const *raster, dal::SpaceDimensions const &dimensions,
                                 LddDrawProps const &properties, QColor const &penColour)

    : RasterDrawer(dimensions, raster), _raster(raster), _properties(properties), _penColour(penColour)
...
void LddRasterDrawer::draw(QPainter &painter, QRect const &indices, QTransform const &world_to_screen,
                           QTransform const &screen_to_world) const
{
  ...
  firstRow = firstRow > 1 ? firstRow - 1 : firstRow;
  ...
  pitSizeX = std::abs(0.5 * _raster->dimensions().cellSize() * this->scale(world_to_screen));
  pitSizeX = std::max(1.0, pitSizeX);
  pitSizeY = std::abs(0.5 * _raster->dimensions().cellSize() * this->scale(world_to_screen));
      ...
      if (!pcr::isMV(matrix.cell<UINT1>(row, col))) {
        ...
        if (value != 5) {
          ...
          if (value == 1) {
            ...
            _raster->dimensions().coordinates(row + 1 + 0.5, col - 1 + 0.5, ncxWld, ncyWld);
          } else if (value == 2) {
            // Next cell to bottom.
            // Calc coordinates of that cell.
            _raster->dimensions().coordinates(row + 1 + 0.5, col + 0.5, ncxWld, ncyWld);
          } else if (value == 3) {
            ...
            // Next cell to left.
            // Calc coordinates of that cell.
            _raster->dimensions().coordinates(row + 0.5, col - 1 + 0.5, ncxWld, ncyWld);
          } else if (value == 6) {
            // Next cell to right.
            // Calc coordinates of that cell.
            _raster->dimensions().coordinates(row + 0.5, col + 1 + 0.5, ncxWld, ncyWld);
          ...
          _raster->dimensions().coordinates(row + 0.25, col + 0.25, leftPitWld, topPitWld);
