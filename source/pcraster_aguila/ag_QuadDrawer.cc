...
//------------------------------------------------------------------------------
ag::BooleanQuadDrawer::BooleanQuadDrawer(Raster const &r, const BooleanDrawProps &dp, size_t l)

    : ClassQuadDrawer(l), d_raster(r), d_dp(dp)
...
void ag::BooleanQuadDrawer::draw(size_t r, size_t c) const
{
  ...
  if (!pcr::isMV(v)) {
    if (v) {
      ...
      glColor3f(falseColour.red() / m, falseColour.green() / m, falseColour.blue() / m);
...
ag::NominalQuadDrawer::NominalQuadDrawer(Raster const &r, const NominalDrawProps &dp, size_t l)
...
ag::OrdinalQuadDrawer::OrdinalQuadDrawer(Raster const &r, const OrdinalDrawProps &dp, size_t l)
...
ag::ScalarQuadDrawer::ScalarQuadDrawer(Raster const &r, const RangeDrawProps &dp, size_t l)

    : RangeQuadDrawer(dp, l), d_raster(r)
...
ag::DirectionalQuadDrawer::DirectionalQuadDrawer(Raster const &r, const RangeDrawProps &dp, size_t l)
...
ag::LddQuadDrawer::LddQuadDrawer(Raster const &r, const LddDrawProps &dp, size_t l)

    : ClassQuadDrawer(l), d_raster(r), d_dp(dp),
      d_gdd(d_raster.dimensions().nrRows(), d_raster.dimensions().nrCols(),
            d_raster.dimensions().cellSize(), d_raster.dimensions().west(),
            d_raster.dimensions().north(), dal::TypeTraits<unsigned char>::typeId)
