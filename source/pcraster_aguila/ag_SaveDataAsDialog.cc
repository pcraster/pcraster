...
bool SaveDataAsDialog::nameIsValid(QString const &name)
{
  return !name.isEmpty();
}
...
SaveDataAsDialog::SaveDataAsDialog(dal::Formats const &formats, QWidget *parent, bool modal,
                                   Qt::WindowFlags flags)

    : QDialog(parent, flags), d_formats(formats)

{
  setModal(modal);
  d_ui.setupUi(this);
  connect(d_ui.d_cancelButton, SIGNAL(clicked()), this, SLOT(reject()));
  connect(d_ui.d_saveButton, SIGNAL(clicked()), this, SLOT(accept()));
  connect(d_ui.d_browseButton, SIGNAL(clicked()), this, SLOT(browse()));
  connect(d_ui.d_nameEdit, SIGNAL(textChanged(QString)), this, SLOT(nameChanged(QString)));
...
void SaveDataAsDialog::nameChanged(const QString &name)
{
...
{
  return dal::addExtensionIfNeeded(std::string(d_ui.d_nameEdit->text().trimmed().toUtf8().constData()),
