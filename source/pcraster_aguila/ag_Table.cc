...
*/
Table::Table(std::string const &name, dal::DataSpace const &space)
...

{
  ...
  {
    std::tuple<std::string, std::vector<std::string>> tuple = dal::splitNameAndSelection(this->name());

    if (!std::get<1>(tuple).empty()) {
      if (std::get<1>(tuple).size() != 2) {
        std::string const message = std::format("Selection specification of {0} ({1}): "
                                                "Must contain two indices.\n",
                                                this->name(), dal::dataSpaceToString(space));
        throw com::Exception(message);
      } else {
        selectedTimeCol = boost::lexical_cast<size_t>(std::get<1>(tuple)[0]);
        selectedAttrCol = boost::lexical_cast<size_t>(std::get<1>(tuple)[1]);

        if (selectedTimeCol == 0 || selectedTimeCol > table->nrCols() || selectedAttrCol == 0 ||
            selectedAttrCol > table->nrCols()) {
          std::string const message =
              std::format("Selection specification of {0} ({1}): "
                          "Indices must be in range [1, {2}].\n",
                          this->name(), dal::dataSpaceToString(space), table->nrCols());
          throw com::Exception(message);
        }
  ...
  if (selectedAttrCol < table->nrCols()) {
    ...
    d_attrCol = table->indexOf(std::filesystem::path(name).filename().string());
  ...
  if (d_attrCol == d_timeCol) {
    std::string const message =
        std::format("Attribute column {0} from {1} ({2}): Same as time column.\n"
                    "Use a different column as attribute column",
                    (d_attrCol + 1), this->name(), dal::dataSpaceToString(space));
    throw com::Exception(message);
  }
  ...
  if (!dal::isInteger(table->typeId(d_timeCol))) {
    std::string const message = std::format("Time column {0} from {1} ({2}): Values have type {3}.\n"
                                            "Valid types are integer types",
                                            (d_timeCol + 1), this->name(), dal::dataSpaceToString(space),
  ...
  if (!dal::isNumeric(table->typeId(d_attrCol))) {
    std::string const message =
        std::format("Attribute column {0} from {1} ({2}): Values have type {3}.\n"
...
void Table::read(dal::DataSpace const &space, dal::DataSpaceAddress const &address)
{
  assert(d_table);
  ...
  for (size_t i = 0; i < dataSource().dataSpace().rank(); ++i) {
    if (dataSource().dataSpace().dimension(i).meaning() != dal::Time && !localAddress.isValid(i)) {
    ...
    if (!dataSource().dataSpace().contains(localAddress)) {
      ...
      if (mapper != nullptr) {
        ...
        std::transform(col.begin(), col.end(), col.begin(),
                       [capture0 = static_cast<UINT4>(timeStepOffset)](auto &&PH1) {
                         return std::plus<UINT4>()(std::forward<decltype(PH1)>(PH1), capture0);
                       });
      }
...
/*!
  ...
  \sa        .
*/
bool Table::isRead(dal::DataSpaceAddress const &address) const
{
  bool result = false;
  ...
  if (isRead()) {
    ...
    result = enclosingSpace.equal(enclosingSpace.trim(space, addressRead()),
...
template <>
bool Table::value<std::string>(std::string &result, dal::DataSpace const &space,
                               dal::DataSpaceAddress const &address) const
{
  dal::DataSpaceAddress const trimmedAddress = dataSource().dataSpace().trim(space, address);
  result = "TODO";
