...
template <class T> com_ClassClassifier<T>::com_ClassClassifier()
{
}
...
template <class T> void com_ClassClassifier<T>::clearClasses()
{
...
/*!
  ...
  \sa        .
*/
template <class T> void com_ClassClassifier<T>::setClasses(int first, int last)
{
  assert(last >= first);
...
*/
template <class T> void com_ClassClassifier<T>::setClasses(const std::vector<T> &c)
{
...
/*!
  \param c The classes to copy.
*/
template <class T> void com_ClassClassifier<T>::setClasses(const std::set<T> &c)
{
  //  1. Resize the d_classes datastructure.
  //  2. Copy the class numbers.
  //  3. Fill the description fields.
...
*/
template <class T> void com_ClassClassifier<T>::setClasses(const T *c, size_t n)
{
...
template <class T>
void com_ClassClassifier<T>::setClasses(T const *values, std::string const *labels, size_t size)
{
  clearClasses();
...
template <class T> void com_ClassClassifier<T>::setClasses(const std::vector<com_LegendClass<T>> &c)
{
  d_classes = c;
}
...
*/
template <class T> size_t com_ClassClassifier<T>::nrClasses() const
{
...
*/
template <class T> std::vector<T> com_ClassClassifier<T>::classNumbers() const
{
...
template <class T> const std::vector<com_LegendClass<T>> &com_ClassClassifier<T>::classes() const
{
  return d_classes;
}

template <class T> T com_ClassClassifier<T>::value(size_t i) const
{
  assert(nrClasses());
  assert(i < nrClasses());
...
template <class T> std::string com_ClassClassifier<T>::descr(size_t i) const
{
  assert(nrClasses());
  assert(i < nrClasses());
...
template <class T> typename com_ClassClassifier<T>::const_iterator com_ClassClassifier<T>::begin() const
{
  return d_classes.begin();
}

template <class T> typename com_ClassClassifier<T>::const_iterator com_ClassClassifier<T>::end() const
{
