...
template <class T> com_LogClassifier<T>::~com_LogClassifier()
{
}
...
*/
template <class T> void com_LogClassifier<T>::classify(std::vector<T> &b, T min, T max, size_t n)
{
  assert(min < max);
...
template <class T> void com_LogClassifier<T>::autoClassify(std::vector<T> &b, T min, T max, size_t n)
{
  assert(min < max);
