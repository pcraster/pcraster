...
template <class T> com_TLogClassifier<T>::~com_TLogClassifier()
{
}
...
*/
template <class T> void com_TLogClassifier<T>::classify(std::vector<T> &b, T min, T max, size_t n)
{
  assert(min < max);
  assert(n > 0);
...
template <class T> void com_TLogClassifier<T>::autoClassify(std::vector<T> &b, T min, T max, size_t n)
{
