...
BOOST_AUTO_TEST_CASE(dal_formats)
{
...

  {
    ...
    BOOST_TEST(environment.formatNames()[0] == "PCRaster");
  }

  {
    ...
    BOOST_TEST(environment.formatNames()[0] == "PCRaster");
    BOOST_TEST(environment.formatNames()[1] == "HDF4");
    BOOST_TEST(environment.formatNames()[2] == "ESRI Shapefile");
  }
#endif
}
...
BOOST_AUTO_TEST_CASE(gdal_data)
{
  ...
  // Set variable if it is not already set.
  {
    ...
    BOOST_TEST(environment.gdalData() == "/usr/share/gdal");
  }
  ...
  // Don't touch variable if it is already set.
  {
    ...
    Environment const environment("/usr");
    BOOST_TEST(environment.gdalData() == "");
  }
  ...
  // Don't touch variable if it is already set.
  {
    dev::setEnvironmentVariable("GDAL_DATA", " ");
    Environment const environment("/usr");
    BOOST_TEST(environment.gdalData() == std::string(" "));
  }
#endif
}
