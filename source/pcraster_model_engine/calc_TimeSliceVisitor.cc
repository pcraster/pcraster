...
TimeSliceVisitor::TimeSliceVisitor(LddGraph const &lg, VField<INT4> const &nrTimeSlices,
                                   Field const &timestepInSecs)
    : DownstreamVisitor(lg), d_nrTimeSlicesField(nrTimeSlices)
{
...
void TimeSliceVisitor::visitPerCachmentSlice()
{
  ...
  for (auto c = graph().catchmentsBegin(); c != graph().catchmentsEnd(); ++c) {
...
void calc::TimeSliceVisitor::visitCatchmentOfPit(LddGraph::Catchments::const_iterator c)
{
