...
void calc::ASTSymbolInfo::init()
{
  ...
  d_ioType = IOType(pcrxml::ModelInputType::Initial, pcrxml::ModelOutputType::Fixed);
...
calc::ASTSymbolInfo::ASTSymbolInfo(const DataType &dataType, const ASTPar *first)
{
  init();
  d_dataType = dataType;
...
std::string calc::ASTSymbolInfo::vsClashError(const calc::VSClash &v,
...
const char *calc::ASTSymbolInfo::str(DefinitionCreation dc)
{
  switch (dc) {
    ...
    case Interface:
      return "interface section";
    case Definition:
      return "xml definition wrapper";
    case Resolve:
    default:
      return "resolve()";
  }
...
/*! \brief resolve data requirements, finding and checking files, comparing
 ...
 \todo   getting messy this code, maybe put each resolve in type: StackInput,
          LookupTable etc.
 */
void calc::ASTSymbolInfo::resolve(IOStrategy &ios)
{
 ...
 */
void calc::ASTSymbolInfo::setReport(const ASTPar *reportPar, const Report *report, bool inDynamic,
...
#define X_IF_PRESENT_RETURN_GET2(x1, x2) X_IF_PRESENT_RETURN2(x1, x2, d_definition->x1()->x2().get())
...
{
  throw SymException(*(d_firstCodePresence->position()), errorMsgName(), e.messages());
}
...
void calc::ASTSymbolInfo::throwSym(const Position &pos, const std::string &msg) const
{
