 ...
 */
calc::TestAsciiResult::TestAsciiResult(const std::string &fName) : d_fName(fName)
{
  com::remove(fName);
}
 ...
 * and u+
 */
calc::ASTExpr *calc::ASTTestFactory::createExpr(const std::string &opName)
{
...
calc::ASTPar *calc::ASTTestFactory::createPar(const std::string &name)
{
  return new ASTPar(name);
}

calc::ASTNumber *calc::ASTTestFactory::createNumber(const std::string &numericString)
{
  return new ASTNumber(numericString);
}

//! create assignment
calc::ASTAss *calc::ASTTestFactory::createAss(const std::string &parName, ASTNode *value,
                                              const Position *pos)
{
  auto *a = new ASTAss();
  ASTPar p(parName);
...
calc::ASTNode *calc::ASTTestFactory::createCode(const char *xmlCode)
{
...
namespace calc
{
static std::string optAttr(const QDomElement &e, const char *attrName, const char *valueOnAbsence)
{
...
calc::ASTNode *calc::ASTTestFactory::createCode(const QDomElement &e)
{
  ASTNode *n(nullptr);
...
bool calc::ASTTestFactory::msgVerify(const std::string &id, const com::Exception &e,
                                     const std::string &prefix)  // const
{
  return db().equals(id, e, prefix);
}

bool calc::ASTTestFactory::fileVerify(const std::string &id,
                                      const std::string &createdFile)  // const
{
  return db().equalsFile(id, createdFile);
}

calc::ASTNode *calc::ASTTestFactory::createFromId(const std::string &id)
{
  if (db().hasXML(id)) {
...
calc::ASTNode *calc::ASTTestFactory::createFromId(const char *id)
{
  return createFromId(std::string(id));
}

std::string calc::ASTTestFactory::modelFromId(const std::string &id)
{
  return db().model(id);
}

calc::ASTScript *calc::ASTTestFactory::createFromIdOrStr(const std::string &codeOrId)
{
