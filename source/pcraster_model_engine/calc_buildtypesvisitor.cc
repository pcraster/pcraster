...
static std::string argSTError(std::string const &name, STClash const &s, size_t nr)
{
  std::string legal;
  std::string illegal;
...
static std::string vsError(const std::string &offender, const calc::VSClash &v)
{
  std::ostringstream msg;
...
static std::string argVSError(const calc::Operator &o, const calc::VSClash &v, int nr)
{
  return vsError(o.strInput(nr), v);
}

static std::string argCombError(const calc::Operator &o, const calc::VSClash &v, int nr)
{
  std::ostringstream msg;
...
std::string assError(const calc::VSClash &v, const calc::ASTSymbolInfo &p)
{
  return p.vsClashError(v, "redefined");
}

static std::string useError(const calc::VSClash &v)
{
  std::ostringstream msg;
  msg << "used as " << v.mustBeOneOf() << " type, but has " << v.isOneOf() << " type";
  return msg.str();
}
...
public:
   ...
   */
  TopDownExprRestrictor(ASTSymbolTable &table, const DataType &r) : d_table(table)
  {
    d_dt.push(r);
  }
  ...
  void subTypeDataStorage(ASTPar *p, const BaseExpr *e, const Operator &op, const DataType &dtExpr)
  {
    ...
    switch (ovs) {
      ...
      case VS_MAPSTACK: {
        ...
        switch (op.opCode()) {
          case OP_TIMEINPUT:
            mst.setUse(MapStackType::Full);
            break;
          case OP_TIMEINPUTSPARSE:
            mst.setUse(MapStackType::Sparse);
            break;
          case OP_LOOKUPMAPSTACK:
            mst.setUse(MapStackType::Lookup);
            break;
...
calc::BuildTypesVisitor::BuildTypesVisitor(CFGNode *cfg) : CFGVisitor(cfg)
{
}
  ...
  try {
    for (i = 0; i < o->nrArgs(); ++i) {
      ...
      if (i > op.polyEqualityInputBegin()) {
        ...
        d_dtc.restrict(o->arg(i)->returnDataType(), o->arg(i - 1)->returnDataType().vs());
        // bubble back
        d_dtc.restrict(o->arg(i - 1)->returnDataType(), o->arg(i)->returnDataType().vs());
      }
...
//! evaluate   p=expr
void calc::BuildTypesVisitor::singleAss(ASTPar *p, const DataType &dtExpr)
{
