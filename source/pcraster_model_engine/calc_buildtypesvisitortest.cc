...
struct Fixture : public calc::LibraryClassNoQt {
  ...
  void insertTestTable(calc::ASTSymbolTable *d_inputTable, std::vector<calc::ASTPar *> d_tt_pars,
                       const std::string &name, const calc::DataType &ft)
  {
    ...
    (*d_inputTable)[p].setIoType(IOType(pcrxml::ModelInputType::None, pcrxml::ModelOutputType::Fixed));
  }
...
BOOST_AUTO_TEST_CASE(testModel)
{
  ...
  {  // test setFirstAss
    ASTCFGTester const n(StringParser::createCodeAsNode("a=1;a=2"));
    BuildTypesVisitor btv(n.cfg());
    btv.visit();
  ...
  {
    N_BTV_VISIT(StringParser::createCodeAsNode("tmp.res=cover(cover.Expr1.imap,sqrt(9));"));
    // visit i=0 in macro
