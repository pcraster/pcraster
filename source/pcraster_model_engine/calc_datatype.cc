...
calc::DataType::DataType(DataType const &rhs)
    : d_vs(rhs.d_vs), d_st(rhs.d_st), d_allowPromotion(rhs.d_allowPromotion), d_unit(rhs.d_unit),
      d_resultType(rhs.d_resultType), d_mapStackType(rhs.d_mapStackType),
 ...
 */
void calc::DataType::restrict(const DataType &req)
{
  // common for all
  restrict(req.vs());
...
std::ostream &calc::operator<<(std::ostream &s, const calc::DataType &ft)
{
  s << "d_vs(" << ft.vs() << ")";
  const char *st("error");
  switch (ft.st()) {
