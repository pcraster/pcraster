...
{
public:
  DynamicWaveExecArguments(const Operator &op, RunTimeEnv *rte, size_t nrActualArgs)
...
private:
  ...
  double dwLookup(size_t v, double fromVal, LookupColumns fromCol, LookupColumns toCol) const;

public:
  DWVisitor(double *state, double *flux, const LookupTable &tab, const VField<INT4> &profileId,
            const LddGraph &lg, const float *inflow, const VField<float> &bottomLevel,
            const VField<float> &roughness, const VField<float> &segmentLength,
            const VField<INT4> &nrTimeSlices, const Field &timestepInSecs,
...
DWVisitor::DWVisitor(double *state, double *flux, const LookupTable &tab, const VField<INT4> &profileId,
                     const LddGraph &lg, const float *inflow, const VField<float> &bottomLevel,
                     const VField<float> &roughness, const VField<float> &segmentLength,
                     const VField<INT4> &nrTimeSlices, const Field &timestepInSecs,
                     const VField<UINT1> &constantState)
    : TimeSliceVisitor(lg, nrTimeSlices, timestepInSecs), d_state(state), d_flux(flux),

      d_tab(tab), d_profileId(profileId), d_inflow(inflow), d_bottomLevel(bottomLevel),
      d_roughness(roughness), d_segmentLength(segmentLength), d_constantState(constantState),
...
double tableLookup(const LookupTable &tab, INT4 profileVal, float fromVal,
                   DynamicWaveTable::LookupColumns fromCol, DynamicWaveTable::LookupColumns toCol)
{
  double result = NAN;
  ...
  if (!r) {
    ...
    const char *names[] = {"ProfileId", "H", "A", "P"};
...
double DWVisitor::qDynChezy(double Roughness, double Aact, double Sf, double R)
{
...
//! calculates Q in m3/sec
double DWVisitor::qDynManning(double Roughness, double Aact, double Sf, double R)
{
...
void calc::DynamicWave::exec(RunTimeEnv *rte, const Operator &op, size_t nrActualInputs) const
{
...
void calc::LookupState::exec(RunTimeEnv *rte, const Operator &op, size_t nrActualInputs) const
{
  DynamicWaveExecArguments a(op, rte, nrActualInputs);
  const auto *tab = dynamic_cast<const LookupTable *>(a.firstNonFieldInput());
...
void calc::LookupPotential::exec(RunTimeEnv *rte, const Operator &op, size_t nrActualInputs) const
{
  DynamicWaveExecArguments a(op, rte, nrActualInputs);
  const auto *tab = dynamic_cast<const LookupTable *>(a.firstNonFieldInput());
...
static double iterateToQnew(double Qin, double Qold, double q, double alpha, double beta,
                            double sliceInSecs, double deltaX);
...
\endcode
*/
void calc::Kinematic::exec(RunTimeEnv *rte, const Operator &op, size_t nrArgs) const
{
  ...
  struct KWVisitor : public TimeSliceVisitor {
    ...
    KWVisitor(Field &qNew, VField<INT4> const &nrTimeSlices, Field const &timestepInSecs,
              ExecArguments const &arg, LddGraph const &lg)
        :

          TimeSliceVisitor(lg, nrTimeSlices, timestepInSecs), d_Qnew(qNew.dest_f()),
          d_QSumDownStream(qNew.nrValues()),

          d_q(arg[2], qNew.nrValues()), d_alpha(arg[3], qNew.nrValues()),
          d_beta(arg[4], qNew.nrValues()), d_deltaX(arg[7], qNew.nrValues())
    {
...
void calc::KinematicWave::exec(RunTimeEnv *rte, const Operator &op, size_t nrArgs) const
{
  ...
  struct KWFSVisitor : public TimeSliceVisitor {
    ...
    KWFSVisitor(Field &newState, Field &flux, VField<INT4> const &nrTimeSlices,
                Field const &timestepInSecsField, ExecArguments const &arg, LddGraph const &lg)
        :
          ...
          d_QSumDownStream(newState.nrValues()), d_qChan(arg[2], newState.nrValues()),
          d_alpha(arg[3], newState.nrValues()), d_beta(arg[4], newState.nrValues()),
  ...
  struct DivideFluxByNrTimeSlices : public DownstreamVisitor {
    ...
    DivideFluxByNrTimeSlices(Field &flux, VField<INT4> const &nrTimeSlices, LddGraph const &lg)
        :
          ...
          DownstreamVisitor(lg), d_flux(flux.dest_f()), d_nrTimeSlices(nrTimeSlices)
    {
    }
  };
...
static double iterateToQnew(double Qin,  /* summed Q new in for all sub-catchments */
                            double Qold, /* current discharge */
                            double q, double alpha, double beta, double sliceInSecs, double deltaX)
{
