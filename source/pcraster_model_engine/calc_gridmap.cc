...
calc::GridMapOut::GridMapOut(std::string const &fileName, dal::RasterDriver const &driver,
                             geo::RasterSpace const &rs, VS vs)
    : GridMap(fileName), d_driver(driver)
{
  d_rs = rs;
  d_vs = vs;
}
...
calc::GridStat calc::GridMapOut::writeData(const void *allValues)
{
  ...
  dal::Raster raster(d_rs.nrRows(), d_rs.nrCols(), d_rs.cellSize(), d_rs.west(), d_rs.north(), t);
...
calc::GridMapIn::GridMapIn(std::string const &fileName) : GridMap(fileName)
{
  ...
  d_rs = geo::RasterSpace(raster->nrRows(), raster->nrCols(), raster->cellSize(), raster->west(),
                          raster->north(), geo::csfProjToGeo(prj), angle);
  assert(driver);
