 ...
 */
calc::LddGraph::LddGraph(const UINT1 *lddField, const IFieldRDConversion &conv)
        ...
        while (!targets.empty()) {
          ...
          for (geo::NB::Code nb = 0; nb < 8; ++nb) {
            ...
            if (sourceR != conv.invalidId()) {
              ...
              if (sourceF != conv.invalidId() && lddField[sourceF] != MV_UINT1 &&
 ...
 */
calc::LddGraph::LddGraph(const LddGraph &org, const boost::dynamic_bitset<> &isMV,
                         bool propagateDownstream)
    : RasterGraph(org.nrVertices()), d_invalidFieldId(org.d_invalidFieldId)
{
