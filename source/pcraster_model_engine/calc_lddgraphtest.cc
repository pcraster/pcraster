...
BOOST_AUTO_TEST_CASE(testUpstream)
{
  ...
  UINT1 lddField[8] = {6, 5, 4, 4,   // -> P <-<-
                       6, 6, 6, 8};  // -> ->->|^
  REAL4 resultR[8] = {1, 0, 1, 2, 6, 5, 4, 3};
...
BOOST_AUTO_TEST_CASE(testDownstreamIterator)
{
  ...
  REAL4 resultR[8] = {0, 7, 5, 4, 0, 1, 2, 3};
...
BOOST_AUTO_TEST_CASE(testInitField)
{
  ...
  UINT1 lddField[8] = {6,        5, 4, 4,   // -> X <-<-
                       MV_UINT1, 6, 6, 8};  // .  ->->|^
  REAL4 result[8] = {1, 1, 1, 1, 1, 1, 1, 1};
...
struct DownstreamVisitorTester : public DownstreamVisitor {
  ...
  DownstreamVisitorTester(LddGraph const &lg)
      : DownstreamVisitor(lg), d_pitOfMyCatchment(9, 10)  // 10 as not visited marker
  {
...
#define FIELD_SIZE 9
  ...
  struct T : public TimeSliceVisitor {
    ...
    T(const LddGraph &ldd, VField<INT4> const &nrTimeSlices, Field const &timestepInSecs)
        : TimeSliceVisitor(ldd, nrTimeSlices, timestepInSecs), catchmentSliceInSecs(FIELD_SIZE, 0.0F),
          nrInitVertexCalls(FIELD_SIZE, 0), nrFinishVertexCalls(FIELD_SIZE, 0),
