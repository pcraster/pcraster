...
public:
  OperatorCreator(pcrxml::LinkInLibraryManifest const &manifest) : d_manifest(manifest)
  {
  }
...
LinkInExpr::LinkInExpr(Id const &nameBefore, Id const &nameAfter, std::string const &stringArgument)
    : BaseExpr(nameBefore.position(), nameBefore.name() + "::" + nameAfter.name()),
      d_nameBefore(nameBefore), d_nameAfter(nameAfter), d_stringArgument(stringArgument)
{
...
pcrxml::CallPoint LinkInExpr::callPoint() const
{
  ...
  if (isFunction()) {
    ...
    cp.object(pcrxml::CallPoint::object_type(pcrxml::Identifier(d_className),     // className
