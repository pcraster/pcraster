 ...
 *
 */
calc::MaskPacking::MaskPacking(const geo::RasterDim &rs, const Mask &mask)
    : SpatialPacking(rs), d_compressedToDecompressed(rs.nrCells()),
      d_decompressedToCompressed(rs.nrCells()), d_rlIndex(rs.nrCells())
{
...
const calc::Field *calc::MaskPacking::unpack(const Field *f) const
{
  ...
  switch (f->cri()) {
    case CRI_1:
      decompress<>(r->dest_1(), f->src_1());
      break;
    case CRI_4:
      decompress<>(r->dest_4(), f->src_4());
      break;
    case CRI_f:
      decompress<>(r->dest_f(), f->src_f());
      break;
    default:
      PRECOND(false);
  }
...
calc::Field *calc::MaskPacking::pack(const Field *f) const
{
  ...
  switch (f->cri()) {
    case CRI_1:
      compress<>(s->dest_1(), f->src_1());
      break;
    case CRI_4:
      compress<>(s->dest_4(), f->src_4());
      break;
    case CRI_f:
      compress<>(s->dest_f(), f->src_f());
      break;
    default:
      PRECOND(false);
  }
...
template <typename T> void calc::MaskPacking::compress(T *dest, const T *src) const
{
...
template <typename T> void calc::MaskPacking::decompress(T *dest, const T *src) const
{
  for (size_t i = 0; i < d_rlIndex.size() - 1; ++i) {
    size_t const n = d_rlIndex[i + 1] - d_rlIndex[i];
    if (i % 2 /*==odd*/ != d_evenIsValueRL) {
