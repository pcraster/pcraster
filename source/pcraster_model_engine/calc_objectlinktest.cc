...
void calc::CalcLibDemoObjectLink::testOrder(REAL4 *result, const std::vector<const REAL4 *> &dems,
                                            UINT1 singleLdd)
{
  result[0] = dems[0][0];
  result[1] = dems[1][0];
...
void calc::CalcLibDemoObjectLink::testOrder2(std::vector<REAL4 *> &result,
...
BOOST_AUTO_TEST_CASE(testExec)
{
  ...
  auto *t = new ObjectLinkProxy<CalcLibDemoObjectLink>("", rte.rasterSpace(), 0);
...
BOOST_AUTO_TEST_CASE(testCheckAndExec)
{
  ...
  try {
    ...
    BOOST_CHECK(e.messages().find("'CalcLibDemoObjectLink' too many arguments specified") !=
                std::string::npos);
    catched = true;
  }
  BOOST_CHECK(catched);
