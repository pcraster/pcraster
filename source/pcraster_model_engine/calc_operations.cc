 ...
 */
void calc::Operations::add(MAJOR_CODE mrf, MAJOR_CODE oneOf1, MAJOR_CODE oneOf2)
{
  MRFRelatives r{};
  r.d_mrf = mrf;
...
void calc::Operations::init()
{
  ...
  static IfThenElse builtIn_ifthenelse(&foAr_ifthenelse_1, &foAr_ifthenelse_4, &foAr_ifthenelse_f);
...
void calc::Operations::load(const CalcLib::GetMeta &gm)
{
  ...
  if (d_nameOp.find(className) != d_nameOp.end()) {
    throw com::Exception(std::format("objectlink name '{0}' not unique", className));
  }
