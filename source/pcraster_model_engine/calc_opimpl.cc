...
calc::SameUn::SameUn(const ISameUn *fieldOp) : d_fieldOp(fieldOp)
{
}
...
calc::SameBin::SameBin(const ISameBin *op1, const ISameBin *op2, const ISameBin *op3) : d_fieldOp(3)
{
...
void calc::DiffBin::exec(RunTimeEnv *rte, const Operator &op, size_t DEBUG_ARG(nrArgs)) const
{
  ...
  switch (b.t) {
    case B::SS_NN:
      d_fieldOp[i]->ss(r, a[0].src(), a[1].src(), b.n);
      break;
    case B::SN:
      d_fieldOp[i]->sn(r, a[0].src(), a[1].src(), b.n);
      break;
    case B::NS:
      d_fieldOp[i]->ns(r, a[0].src(), a[1].src(), b.n);
      break;
  }
...
calc::DiffBin::DiffBin(const IDiffBin *op1, const IDiffBin *op2, const IDiffBin *op3) : d_fieldOp(3)
{
  initOp(d_fieldOp, op1);
  initOp(d_fieldOp, op2);
  initOp(d_fieldOp, op3);
...
calc::IfThenElse::IfThenElse(const IIfThenElse *op1, const IIfThenElse *op2, const IIfThenElse *op3)
    : d_fieldOp(3)
{
  initOp(d_fieldOp, op1);
  initOp(d_fieldOp, op2);
...
calc::DiffUn::DiffUn(const IDiffUn *op1, const IDiffUn *op2, const IDiffUn *op3) : d_fieldOp(3)
{
  initOp(d_fieldOp, op1);
  initOp(d_fieldOp, op2);
  initOp(d_fieldOp, op3);
...
calc::SpatialImpl::SpatialImpl(const IDiffUn *op1, const IDiffUn *op2, const IDiffUn *op3)
    : DiffUn(op1, op2, op3)
{
}
...
void calc::GenNonSpatial::exec(RunTimeEnv *rte, const Operator &op, size_t nrArgs) const
{
  ...
  switch (op.opCode()) {
    case OP_CELLLENGTH:
      gsf.celllength(r.dest_f());
      break;
    case OP_CELLAREA:
      gsf.cellarea(r.dest_f());
      break;
    case OP_MAPUNIFORM:
      gsf.mapuniform(r.dest_f());
      break;
    case OP_MAPNORMAL:
      gsf.mapnormal(r.dest_f());
      break;
    case OP_TIME:
      *(r.dest_f()) = rte->timer().currentInt();
      break;
    case OP_TIMESLICE:
      *(r.dest_f()) = 1;
      break;
...
void calc::GenSpatial::exec(RunTimeEnv *rte, const Operator &op, size_t nrArgs) const
{
  ...
  switch (op.opCode()) {
    case OP_UNIQUEID:
      gsf.uniqueid(r.dest_f());
      break;
    case OP_XCOORDINATE:
      gsf.xcoordinate(r.dest_f());
      break;
    case OP_YCOORDINATE:
      gsf.ycoordinate(r.dest_f());
      break;
    case OP_NORMAL:
      gsf.normal(r.dest_f());
      break;
    case OP_UNIFORM:
      gsf.uniform(r.dest_f());
      break;
    default:
      PRECOND(false);
  }
...
calc::Trig::Trig(const ISameUn *fieldOp) : d_fieldOp(fieldOp)
{
}
...
void areaStatisticalOperation(RunTimeEnv *rte, const Operator &op, size_t nrArgs)
{
  ...
  switch (args[1].cri()) {
    case CRI_1:
      ao.apply(r, args[1].src_1(), args[1].nrValues());
      break;
    case CRI_4:
      ao.apply(r, args[1].src_4(), args[1].nrValues());
      break;
    default:
      POSTCOND(false);
  }
...
void calc::ArgOrderWithIdAddAreaLimited::exec(RunTimeEnv *rte, const Operator &op, size_t nrArgs) const
{
  ExecArguments args(op, rte, nrArgs);
