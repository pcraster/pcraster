...
public:
  OrderSortFO(IVSpatial<double> const &val) : d_val(&val)
  {
  }
...
public:
  AreaOrderSortFO(IVSpatial<double> const &exprVal, IVSpatial<INT4> const &classVal)
...
void calc::orderOperation(REAL4 *res, IVSpatial<double> const &val, size_t len)
{
...
void calc::areaOrderOperation(REAL4 *res, IVSpatial<double> const &expr,
                              IVSpatial<INT4> const &areaClass, size_t len)
{
  std::vector<size_t> order;
  // maximum size to can be used:
  order.reserve(len);
