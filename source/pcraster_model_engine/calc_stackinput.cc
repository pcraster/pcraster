...
calc::StackInput::StackInput(IOStrategy &fios, const std::string &stackName, const MapStackType &type)
    : d_stackName(stackName), d_fios(fios), d_fs(fios.ioFieldStrategy()),
      d_itemToLoad(fios.timer().lastInt() + 1, 0), d_type(VS_FIELD, ST_SPATIAL), d_cachedField(nullptr),
      d_cachedTimeStep(0)
{
  // refactor this stuff
...
calc::StackInput::StackInput(const StackInput &rhs)
    : DataValue(rhs), d_stackName(rhs.d_stackName), d_fios(rhs.d_fios), d_fs(d_fios.ioFieldStrategy()),
      d_itemToLoad(rhs.d_itemToLoad), d_type(rhs.d_type),
      d_cachedField(com::non0Clone(rhs.d_cachedField)), d_cachedTimeStep(rhs.d_cachedTimeStep)
{
}
...
VS calc::StackInput::checkMap(size_t i)
{
  ...
  try {
    ...
    try {
      ...
      std::ostringstream s;
      s << " does not have the same data type as first element (" << d_type.vs() << ")";
