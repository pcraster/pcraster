...
private:
  ...
  void scalarCrossHeader(std::ostream &out, int nrRowDescriptors = 1) const;

  template <class IntervalMapT> void scalarScalarTable(const REAL4 *subject, size_t nrValues) const;
  void GGTable(const REAL4 *subject) const;

  template <typename CountMap> void addSubjectClasses(CountMap &m) const;
  template <typename CountMap> void addCrossClasses(CountMap &m) const;
  template <typename SubjectType, typename CrossType>
  void classCrossTable(const SubjectType *subject) const;
  template <typename SubjectType> void classScalarTable(const SubjectType *subject) const;
  template <class IntervalMapT, typename SubjectType>
  void classIntervalTable(const SubjectType *subject) const;

  template <typename T> void classTable(const T *begin, const T *end) const;
  void scalarTable(const REAL4 *begin, const REAL4 *end) const;
  void scalarTable(const REAL4 *beginS, const REAL4 *endS, const REAL4 *beginC, const REAL4 *endC) const;
  template <typename T> void classSubject() const;
...
public:
  ...
  // CREATORS
  //----------------------------------------------------------------------------
  StatComputation(std::ostream &out, bool verbose, Field const *subjectField,
                  Intervals const *subjectTable, std::string const &subjectName, Field const *crossField,
                  Intervals const *crossTable, std::string const &crossName);
...
struct CS_CP {
  ...
  CS_CP(INT4 subject, REAL4 cross) : d_subjectI(subject), d_cross(cross)
  {
  }

  CS_CP(REAL4 subject, REAL4 cross) : d_subjectR(subject), d_cross(cross)
  {
  }
...
struct ScalarStats : public com::AverageSdMinMax<REAL4> {
  ...
  template <typename KeyT> void printLine(const KeyT &key, double area, std::ostream &d_out) const
  {
    d_out << key;
...
static ScalarStats scalarStats(const REAL4 *begin, const REAL4 *end)
{
  std::vector<REAL4> r;
...
static double median(detail::CS_V::iterator begin, detail::CS_V::iterator end)
{
  return com::median(begin, end, std::mem_fn(&detail::CS_CP::crossLess))->cross();
}
...
static void crossPercentiles(M &m, typename P::V::iterator begin, typename P::V::iterator end)
{
  ...
  for (auto i = m.begin(); i != m.end(); ++i) {
    switch (i->second.nr()) {
      case 0:
        break;  // else d_med keeps 0
      case 1:
      case 2:
        i->second.d_med = i->second.maximum();
        break;
      default:
...
calc::StatTable::StatTable(std::string const &id, InputMap const &subject, InputMap const &cross)
    : BaseExpr("statistics"), d_id(new ASTPar(id)),
      d_copyStringToMemoryOutputId(ASTSymbolInfo::noMemoryExchangeId()), d_subject(subject),
      d_cross(cross)
{
...
void calc::StatTable::exec(RunTimeEnv *rte) const
{
  using I = StatComputation::Intervals;
  ...
  StatComputation const sc(out, d_verbose, subjectField, subjectTable.get(), d_subject.d_name,
                           crossField, crossTable.get(), d_cross.d_name);
  if (d_writeToFile.empty()) {
    // std::cout << out.str();
    rte->transferMemoryExchangeItemIntoDataTransferArray(
        new MemoryExchangeItemString(d_id->name(), d_copyStringToMemoryOutputId, out.str()));
  } else {
...
calc::StatComputation::StatComputation(std::ostream &out, bool verbose, Field const *subjectField,
                                       Intervals const *subjectTable, std::string const &subjectName,
                                       Field const *crossField, Intervals const *crossTable,
                                       std::string const &crossName)
    : d_out(out), d_verbose(verbose)
{
...
template <typename T> void calc::StatComputation::classSubject() const
{
...
template <typename T> void calc::StatComputation::classTable(const T *begin, const T *end) const
{
...
template <typename SubjectType, typename CrossType>
void calc::StatComputation::classCrossTable(const SubjectType *subject) const
{
...
template <class IntervalMapT, typename SubjectType>
void calc::StatComputation::classIntervalTable(const SubjectType *subject) const
{
...
void calc::StatComputation::scalarScalarTable(const REAL4 *subject, size_t nrValues) const
{
  using CP = detail::CS_CP;
...
//! type GG
void calc::StatComputation::GGTable(const REAL4 *subject) const
{
  const REAL4 *cross = d_cross.d_field->src_f();
  ...
  using MapKey = com::IntervalMultiMap<detail::ScalarStats, Real>;
  using M = com::IntervalMultiMap<MapKey, Real>;
...
void calc::StatComputation::classScalarTable(const SubjectType *subject) const
{
  const REAL4 *cross = d_cross.d_field->src_f();
...
void calc::StatComputation::scalarTable(const REAL4 *beginS, const REAL4 *endS, const REAL4 *beginC,
                                        const REAL4 *endC) const
{
...
void calc::StatComputation::scalarSubject() const
{
  ...
  if (d_subject.d_intervals != nullptr) {
    if (d_cross.d_intervals != nullptr) {
      ...
      using IMM = com::IntervalMultiMap<detail::ScalarStats, Real>;
...
double calc::StatComputation::area(size_t cellCount)
{
