...
calc::SymException::SymException(const Position &pos, const std::string &symbolName,
                                 const std::string &message)
    : PosException(pos.fullText(), symbolName + ": " + message, true), d_position(pos.fullText()),
      d_symbolName(symbolName), d_message(message)
{
}

calc::SymException::SymException(const ASTPar *par, const std::string &message)
    : PosException(par->position()->fullText(), par->name() + ": " + message, true),
      d_position(par->position()->fullText()), d_symbolName(par->name()), d_message(message)
{
}
