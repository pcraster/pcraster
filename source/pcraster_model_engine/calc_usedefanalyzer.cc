...
struct RecordFirst {
  ...
  RecordFirst(P end) : d_end(end), ever(d_end), initial(d_end), dynamic(d_end)
  {
  }
...
public:
  UseDefRecorder(bool prefixFirstUseByDef) : d_prefixFirstUseByDef(prefixFirstUseByDef), d_dummy("dummy")
  {
  }
  ...
  void addPrologue(const UDEvent &e)
  {
  ...
  std::map<std::string, IOType> ioTypes() const
  {
    ...
    for (const auto &i : *this) {
      m.insert(std::make_pair(i.first, i.second.ioType(d_dynamicSection)));
    }
    return m;
  }
};
...
std::map<std::string, calc::IOType> calc::UseDefAnalyzer::ioTypes() const
{
...
std::map<std::string, calc::IOType> calc::ioTypes(CFGNode *cfg)
{
