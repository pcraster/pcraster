...
class Test
{
  ...
  bool checkLastUse(const std::string &path, bool lastUse) const
  {
    ...
    if (!ok) {
      ...
      std::cerr << "lastUse failure path: " << path
                << " script pos: " << astCast<ASTPar>(d_a.ast(), path)->shortPosText();
    }
    return ok;
  }
...
public:
  ...
  template <typename T> T *cast(const char *path) const
  {
...
BOOST_AUTO_TEST_CASE(testLoops)
{
  ...
  {  // use of a AFTER repeat
    const char *code = "               # test path to assignments\n"
                       "               # a b                     \n"
                       "a=0;           # 1    l/0/a              \n"
                       "repeat {       #      l/1/r              \n"
...
BOOST_AUTO_TEST_CASE(testBugs)
{
  ...
  {
    const char *code = " timer 1 3 1; initial tmp2.res=catchmenttotal(inpldd.map,1); "
