...
{
  switch (s) {
    case pcrxml::SpatialType::Spatial:
      return ST_SPATIAL;
    case pcrxml::SpatialType::NonSpatial:
      return ST_NONSPATIAL;
    case pcrxml::SpatialType::Either:
      return ST_EITHER;
  }
  return ST_NON;
}
...
{
  return {xml2VS(f.dataType()), xml2ST(f.spatialType())};
}

template <typename T> static T toXML(DataType const &dt)
{
  T x;
...
pcrxml::SpatialType st2XML(ST st)
{
  switch (st) {
    case ST_SPATIAL:
      return pcrxml::SpatialType::Spatial;
    case ST_NONSPATIAL:
      return pcrxml::SpatialType::NonSpatial;
    case ST_EITHER:
