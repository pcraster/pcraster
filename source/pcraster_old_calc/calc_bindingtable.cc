...
void calc::BindingTable::setExternalBindings(IScript *addToThis, const RunDirectory &rd)
{
  ...
  for (i = extB.begin(); i != extB.end(); ++i) {
    ...
    d_table.insert(std::make_pair(Symbol(addToThis, i->first),
                                  Right(External, Symbol(addToThis, i->second), VS_FIELD)));
  }
}

std::vector<calc::UserSymbol *> calc::BindingTable::moveConstantToParameters(StatementBlock *block)
{
...
calc::BindingTable::Right::Right(DefinitionLevel definitionLevel, const Symbol &value, VS vs)
    : d_definitionLevel(definitionLevel), d_value(value), d_vs(vs)
{
}
