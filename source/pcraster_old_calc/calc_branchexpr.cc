...
calc::BranchExpr::BranchExpr(const Element &pos, const Operator &op, FieldExprArgs &fieldArgs)
    : FieldArgs(pos, op, fieldArgs), FieldExpr(pos), d_type(op)
{
  buildTypes();
}
...
void calc::BranchExpr::defaultBuildType(VS &newVs, bool &isSpatial)
{
  Args &args = fieldArgs();
  if (nrInSet(newVs) > 1) {
...
void calc::BranchExpr::argCombError(int nr, /* offending argument nr        */
                                    VS prevVs) const
{
