...
calc::Constant::Constant(const Symbol &name)
    : FieldExpr(name), d_strRepr(name.name()), d_value(com::fromString<double>(name.name())),
      d_type(vsOfNumber(d_value), ST_NONSPATIAL)
{
}

//! ctor cast like "scalar(v)"
calc::Constant::Constant(const Symbol &castFunctionName, VS castDestination, const Symbol &v)
    : FieldExpr(castFunctionName), d_strRepr(castFunctionName.name()), d_value(v.toNumber()),
      d_type(vsOfNumber(d_value), ST_NONSPATIAL)
{
  try {
