 ...
 */
calc::CsfMap::CsfMap(const std::string &fileName) : GridMap(fileName, 0, 0, VS_FIELD), d_map(fileName)
{
...
calc::CsfMap::CsfMap(const std::string &fileName, const geo::RasterSpace &rs, VS vs)
    : GridMap(fileName, rs.nrRows(), rs.nrCols(), vs), d_map(d_fileName, rs, vs2CsfVs(vs))
{
}
