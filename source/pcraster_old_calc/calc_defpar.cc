...
calc::DefPar::DefPar(const ConstructPar &p) : ParsPar(p)
{
...
void calc::DefPar::initError(const com::Exception &msg) const
{
 ...
 */
calc::SubParameter *calc::DefPar::indexTable(const IndexTable *it, bool constant,
                                             const FieldType &useType) const
{
  if (!(it->arrayDefVector() == descriptor())) {
    // pcrcalc/test277
    std::ostringstream msg;
    msg << it->qName() << " is defined as an index-table on " << it->arrayDefName() << " type on "
        << it->definitionPoint() << " and used here as " << descriptor().name();
  ...
  if (isIn(useType.vs(), VS_FIELD)) {
    if (useType.spatial()) {
      ...
      v = new FieldMapInputParameter(*this, constant, mapVs, vals, it->scriptConst().ioFieldStrategy());
    } else {
