...
#define STATIC_GRIDIO_FPTR(funcName) STATIC_DLL_FUNC_PTR(d_dll, funcName)
...
static com::DynamicLibrary *openGridDll(const std::string &dllDir)
{
...
static com::DynamicLibrary *openGridDll()
{
  ...
  if (!esriArcView3Only) {
    // ArcGis 9 tested
    {
      com::Win32RegistryKey k(com::Win32RegistryKey::LocalMachine, "SOFTWARE\\ESRI\\ArcGIS");
      if (k.exists()) {
 ...
 */
    ...
    if (!d_dll->wasAlreadyLoaded()) {
      IntVoidPtr gridIOSetup = (IntVoidPtr)d_dll->loadFunction("GridIOSetup");
      (*gridIOSetup)();
    }
  } catch (...) {
    clean();
...
void calc::EsriGridIO::gridDelete(const std::string &gridName)
{
  ...
  com::PathInfo pi(pn);
  if (pi.exists()) {
    HANDLE h = CreateFile(pn.toString().c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
...
void calc::EsriGridIO::describeGridDbl(const std::string &grdnam, double *cellsz, int *gridsz,
                                       double *box, double *sta, bool &isFloat, int *nclass, int *reclen)
{
  ...
  switch (dataTypeInt) {
    ...
    case CELLFLOAT:
      ...
      break;
    default:
      throwError("esri violates its api");
  }
}
                                      ...
                                      double cellsize, const double box[4])
{
  ...
  int const chan_id = (*funcPtr)(name.c_str(), rdwrflag, iomode, celltype, cellsize, box);
  if (chan_id < 0) {
...
void calc::EsriGridIO::putWindowCellFloat(int channel, size_t col, size_t row, double fcell)
{
