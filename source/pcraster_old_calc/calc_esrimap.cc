...
calc::NotAnEsriGrid::NotAnEsriGrid() : com::Exception(" not an ESRI grid")
{
}
...
calc::EsriMap::EsriMap(const std::string &fileName) : calc::GridMap(fileName, 0, 0, VS_FIELD)
{
  /*
 ...
 */
  ...
  calc::EsriGridIO::privateAccessWindowSet(d_chanId, d_box, d_cellSize, dummyAdjBndBox, nrRows, nrCols);
  d_nrRows = nrRows;
  d_nrCols = nrCols;
...
calc::EsriMap::Window::Window(int winChan) : d_winChan(winChan)
{
}
...
bool calc::EsriMap::getMinMax(double &min, double &max) const
{
  ...
  d_chanId =
      calc::EsriGridIO::cellLayerOpen(fileName(), READONLY, ROWIO, &cellTypeDummy, &cellSizeDummy);
...
{
  DEVELOP_PRECOND(val);
  float **ptrVal = com::new2d<float>(nrRows(), nrCols(), static_cast<float *>(val));
  float *linVal = ptrVal[0];  // as linear

  calc::EsriGridIO::getWindowBandFloat(d_chanId, 0, static_cast<int>(nrRows()), ptrVal);
...
void calc::EsriMap::readInt(void *val)
{
  ...
  int **ptrVal = com::new2d<int>(nrRows(), nrCols(), static_cast<int *>(val));

  calc::EsriGridIO::getWindowBandInt(d_chanId, 0, static_cast<int>(nrRows()), ptrVal);
