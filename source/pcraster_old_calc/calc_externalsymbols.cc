...
calc::ModelLinkProxy::ModelLinkProxy(const std::string &name, PCR_EXTERNAL_MODELLINK_CREATOR creator)
...
calc::PCR_EXTERNAL_MODELLINK_CREATOR calc::ModelLinkProxy::creator() const
{
  return d_creator;
}
...
calc::ExternalSymbols::~ExternalSymbols()
{
  for (auto &d_librarie : d_libraries) {
    delete d_librarie;
  }
}
...
const calc::ModelLinkProxy *calc::ExternalSymbols::findModelLinkProxy(const std::string &name) const
{
  auto it = std::find_if(d_modelLinkProxies.begin(), d_modelLinkProxies.end(), linkNameIs(name));
...
void calc::ExternalSymbols::loadDynamicLibrary(const std::string &extLibNoExt)
{
  try {
    ...
    using GET_LIST_PTR = PCR_EXTERNAL_FUNCTION_LIST *(*)();
    using MODELLINK_LIST_PTR = PCR_EXTERNAL_MODELLINK_LIST *(*)();
    ...
    try {
      ...
      if (lib->hasSymbol("GetFunctionList")) {
        ...
        GET_LIST_PTR getListPtr = (GET_LIST_PTR)lib->loadSymbol("GetFunctionList");
      ...
      if (lib->hasSymbol("GetModelLinkList")) {
        ...
        MODELLINK_LIST_PTR modelLinkListPtr = (MODELLINK_LIST_PTR)lib->loadSymbol("GetModelLinkList");
      ...
      std::ostringstream str;
      str << "dynamic library " << quote(extLibNoExt) << ", error while loading ";
      e.prepend(str.str());
      throw;
    }
...
*/
const calc::ModelLinkProxy &calc::ExternalSymbols::modelLinkProxy(size_t i) const
{
