...
calc::ExtSym::ExtSym(const std::string &name, const Position *pos)
    : d_name(name), d_pos(pos->createClone())
{
  PRECOND(pos);
}
 ...
 * \param name symbol 'value'
 */
calc::ExtSym::ExtSym(const std::string &name) : d_name(name), d_pos(new PositionNone())
{
}

//! ctor with script Symbol
calc::ExtSym::ExtSym(const Symbol &s) : d_name(s.name()), d_pos(s.position()->createClone())
{
  d_pos->setPriority(1);
}
...
//! Copy constructor.
calc::ExtSym::ExtSym(const ExtSym &e) : d_name(e.d_name), d_pos(e.d_pos->createClone())
{
}
