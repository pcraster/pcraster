...
static void argVsError(const calc::Operator &o, VS allowedVs, VS currentVs,
...
static void argStError(const calc::Operator &o, int nr, /* wrong argument nr        */
                       int offsetArg)
{
  std::string legal;
  std::string illegal;
...
void calc::FieldType::print(calc::InfoScript &i, const std::string &tag) const
{
  ...
  i.stream() << "\n<A HREF=\"#" << tag << "\" onmouseover=showtype(\"" << str << "\")>" << tag << "</A>";
}
