...
calc::FieldValue::FieldValue(const FieldParameter &p, size_t index, Field *val)
    : d_par(p), d_index(index), d_fw(d_par, d_index, p.vs()), d_val(new FieldHandle(val))
{
}

calc::FieldValue::FieldValue(const FieldParameter &p, size_t index)
...
void calc::FieldValue::assign(const FieldHandle &f, const Position *assignPoint)
{
  delete d_val;
