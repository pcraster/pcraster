...
calc::ForEach::ForEach(const calc::Element &pos, calc::StatementBlock *parentBlock,
                       const calc::Symbol &iter, const calc::IdList &in, const calc::IdList &excl,
                       const calc::IdList &order, bool orderIsAscending)
    : calc::InnerStatementBlock(pos, parentBlock), d_symTab(parentBlock), d_iterSymbol(iter),
      d_orderIsAscending(orderIsAscending)
{
  ...
  d_iter = new calc::IndexParameterVariable(calc::BindedSymbol(d_iterSymbol), d_loopedArray, this);
  try {
    addLocal(d_iter);
  } catch (...) {
    delete d_iter;
...
class calc::UserSymbol *calc::ForEach::findSymbol(const class calc::Symbol *sym, VS typesExpected,
...
void calc::ForEach::indexSet(Set &set, const calc::IdList &list)
{
