...
public:
  CoordinateTranslator(const Compressor &c) : d_compressor(c), d_ars(d_compressor.rasterSpace())
  {
  }
  ...
  void get(size_t linIndexCompressed, double &x, double &y) const
  {
    size_t row = 0;
    size_t col = 0;
    d_compressor.rasterSpace().linear2RowCol(d_compressor.toDecompressedIndex(linIndexCompressed), row,
                                             col);
 ...
 */
calc::GenerateSpatialFunc::GenerateSpatialFunc(const UINT1 *maskVal, size_t nrMask, const Compressor &c)
