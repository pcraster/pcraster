...
public:
  LexIndexTable(std::ifstream *f, IndexTable *to) : LexValueFile(f), d_to(to)
  {
  }
...
calc::IndexTable::IndexTable(const calc::Symbol &name, const calc::ArrayDefVector &array)
    : Parameter(name, true), d_array(array)
{
 ...
 */
void calc::IndexTable::throwError(int lineNr, std::ostringstream &what) const
{
 ...
 */
const calc::IndexTable::Value &calc::IndexTable::find(const std::string &parExtName, size_t i) const
{
                                     ...
                                     std::vector<double> &vals) const
{
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    double valD = NAN;
    if ((!CnvrtDouble(&valD, val.d_value.c_str())) || !isIn(vs, vsOfNumber(valD))) {  // pcrcalc/test283
      std::ostringstream msg;
...
VS calc::IndexTable::fieldMapValues(const calc::BindedSymbol &par, std::vector<std::string> &vals) const
{
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    try {
      ...
      if (!isIn(newVs, mapVs)) {
        throw com::Exception(" is a " + toString(newVs) + " map, previous values are of type " +
                             toString(mapVs));  // pcrcalc/test308
      }
      mapVs = newVs;
...
void calc::IndexTable::nameValues(const calc::BindedSymbol &par,
