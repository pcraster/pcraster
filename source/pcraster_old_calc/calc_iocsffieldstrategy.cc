...
calc::IoFieldStrategy *calc::IoCsfFieldStrategy::checkInputMap(VS &vs, const std::string &fName)
{
...
/*!
   callee must delete
 */
const calc::StackReader *calc::IoCsfFieldStrategy::createStackReader(const RunDirectory &rd,
...
calc::GridMap *calc::IoCsfFieldStrategy::createMap(const std::string &fileName, VS vs) const
{
  return new CsfMap(fileName, rasterSpace(), vs);
}

//! return prefix-ItemNumber name
std::string calc::IoCsfFieldStrategy::makeStackItemName(const std::string &iname, int atTimeStep) const
{
  return dal::timeStepPath83(std::filesystem::path(iname), atTimeStep).string();
}
