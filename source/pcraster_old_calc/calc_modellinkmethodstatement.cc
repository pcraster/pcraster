...
calc::ModelLinkMethodStatement::ModelLinkMethodStatement(
    calc::StatementBlock *b, const calc::WriteInfo &w, const calc::Symbol &modelName,
    const calc::Symbol &methodName, const std::vector<calc::UsePar> &left, const std::string &strArg,
    ...
    : calc::Statement(left[0]),
      ...
      d_methodName(methodName), d_args(args), d_sig(strArg, d_args.size())
{
  try {
    buildTypes();
...
void calc::ModelLinkMethodStatement::run()
{
  ...
  for (size_t i = 0; i < d_sig.d_result.size(); i++) {
    ...
    result.push_back(std::make_shared<GlobResult>(vs, vs, scriptConst().compressor()));
