...
calc::ParsIndex::ParsIndex(bool on) : d_on(on)
{
}
...
calc::ParsIndexName::ParsIndexName(bool on, const Symbol &name, const Symbol &extName)
    : calc::ParsIndex(on), d_name(name), d_extName(new calc::Symbol(extName))
{
}
...
calc::ParsIndexSet::ParsIndexSet(bool on, const Symbol &name, const IdList &setList)
...
calc::UserSymbol *calc::ParsIndexName::addMe(calc::ArrayDefinition *a) const
{
  ...
  // while off indices does not, but that doesn't matter
