...
void calc::StatTable::InputMap::swap(InputMap &rhs) noexcept
{
...
void calc::StatTable::InputMap::verbosePrint(std::ofstream &out) const
{
  if (!hasIntervals()) {
    out << "\tniet opgegeven\n";
  } else {
...
struct ScalarStats : public com::AverageSdMinMax<REAL4> {
  ...
  template <typename KeyT> void printLine(const KeyT &key, double area, std::ostream &out) const
  {
    out << key;
...
calc::StatTable::StatTable(FieldExpr *subject) : Statement(*subject), d_resultTable("statTable.txt")
{
  ...
  if (d_cross.defined()) {
    ...
    if (d_subject.cr() == CR_REAL4) {
      if (d_cross.cr() != CR_REAL4) {
        doSwap = true;  // r1,r2
      } else if ((!d_subject.hasIntervals()) && d_cross.hasIntervals()) {
        doSwap = true;  //r3
      }
    }
    if (doSwap) {
...
double calc::StatTable::area(size_t cellCount)
{
...
template <typename T> void calc::StatTable::classSubject(FieldStack &stack) const
{
  ...
  if (!d_cross.defined()) {
    ...
    switch (biggestCellRepr(d_cross->vs())) {
      case CR_INT4:
        classCrossTable<T, INT4>(s, stack);
        break;
      case CR_UINT1:
        classCrossTable<T, UINT1>(s, stack);
        break;
      case CR_REAL4:
...
void calc::StatTable::scalarCrossHeader(std::ostream &out, int nrRowDescriptors) const
{
  crossHeader(out);
  out << d_subject.d_name;
 ...
 */
template <typename CountMap> void calc::StatTable::addSubjectClasses(CountMap &m) const
{
...
template <typename CountMap> void calc::StatTable::addCrossClasses(CountMap &m) const
{
...
void calc::StatTable::classIntervalTable(const SubjectType *subject, FieldStack &stack) const
{
...
//! type GG
void calc::StatTable::GGTable(const REAL4 *subject, FieldStack &stack) const
{
...
template <typename T> void calc::StatTable::popSubject(FieldStack &stack) const
{
  if (d_maskSet) {
...
void calc::StatTable::scalarTable(const REAL4 *begin, const REAL4 *end) const
{
...
void calc::StatTable::scalarTable(const REAL4 *beginS, const REAL4 *endS, const REAL4 *beginC,
