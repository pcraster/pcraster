...
calc::SymbolTable::SymbolTable(calc::StatementBlock *parentBlock) : d_parentBlock(parentBlock)
{
}
...
calc::UserSymbol *calc::SymbolTable::find(const class calc::Symbol *sym, VS typesExpected,
                                          bool mustExist) const
{
  ...
  if (u) {
    if (!isIn(u->symbolType(), typesExpected)) {
      ...
      msg << u->qName() << " is defined as " << toString(u->symbolType()) << " type on "
          << u->definitionPoint() << " and used here as " << toString(typesExpected) << " type";
      sym->posError(msg);
    }
  }
  return u;
...
void calc::SymbolTable::add(calc::UserSymbol *newPar)
{
  try {
    ...
    if (!p.second) {
      ...
      std::ostringstream msg;
      msg << newPar->qName() << " defined twice, first definition at " << firstdef->definitionPoint();
      newPar->posError(msg);
    }
...
calc::SubParameter *calc::SymbolTable::findRightParameter(const calc::ParsPar &par, VS expectedVs) const
{
  return findParameter(par, expectedVs, par.isArray());
}

calc::SubParameter *calc::SymbolTable::findLeftParameter(const calc::ParsPar &par, VS expectedVs) const
{
...
void calc::SymbolTable::createXmlData(std::vector<pcrxml::Data *> &addHere) const
{
 ...
 */
void calc::SymbolTable::setArcViewExtCheckData(std::vector<ArcViewExtCheckData> &r) const
{
