...
calc::Timeoutput::Timeoutput(const calc::WriteInfo &w, const calc::UsePar &par,
                             calc::FieldExprArgs &args)
    : calc::FieldArgs(par, major2op(OP_TIMEOUTPUT), args), calc::Statement(par),
      d_par(new calc::TssOutputParameter(par, w, true)), d_index(par.createSelector())
{
...
{
  const char *msg = "Use a conversion function to pick a data type for second argument of timeoutput "
