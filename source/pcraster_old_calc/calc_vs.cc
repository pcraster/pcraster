...
static std::ostream &operator<<(std::ostream &msg, VS setOfVs)
{
  ...
  const VS_NAME names[] = {
      {VS_S, "scalar"},           {VS_N, "nominal"},       {VS_O, "ordinal"},
      {VS_B, "boolean"},          {VS_L, "ldd"},           {VS_D, "directional"},
      {VS_TABLE, "table"},        {VS_TSS, "timeseries"},  {VS_ARRAY, "array"},
      {VS_INDEX, "array-index"},  {VS_STRING, "string"},   {VS_INDEXTABLE, "indextable"},
      {VS_INDEXSET, "array-set"}, {VS_OBJECT, "modellink"}  // , { VS_UNKNOWN, "unknown!"}
  };
