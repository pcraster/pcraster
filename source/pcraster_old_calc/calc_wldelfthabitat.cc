...
static std::string expectAttr(const QDomElement &e, const QString &attrName)
{
  QString const value = e.attribute(attrName);
  if (value == QString()) {
    std::ostringstream os;
    os << "expect element '" << std::string(e.tagName().toLatin1()) << "' to have attribute '"
...
/*! return atttribute value of attribute \a attrName of
   ...
   * </pre>
   */
static std::string attrOfSubMatch(const QDomElement &tree, const QString &subTagName,
                                  const QString &attrName)
{
  QDomElement const match(pcrxml::firstMatchByTagName(tree, subTagName));
  if (match.isNull()) {
    std::ostringstream os;
    os << "expect element '" << std::string(tree.tagName().toLatin1()) << "' to include element '"
...
static double numericValueOfSubMatch(const QDomElement &e, const QString &tagName)
{
  try {
    return com::fromString<double>(valueOfSubMatch(e, tagName));
  } catch (const std::range_error & /* re */) {
    std::ostringstream os;
    os << "expect element '" << std::string(e.tagName().toLatin1())
...
static std::string subValue(const QDomElement &e)
{
  ...
  if (de.isNull() || !de.hasAttribute("value")) {
    std::ostringstream os;
    os << "expect element '" << std::string(e.tagName().toLatin1()) << "' to include element '"
       << std::string(e.tagName().toLatin1()) << "' with an attribute named value";
    throw com::BadStreamFormat(os.str());
  }
...
static QDomElement expectTagGet1stSibling(const QDomElement &tree, const QString &tag)
{
  QDomElement const e(findTagGet1stSibling(tree, tag));
  if (e.isNull()) {
    std::ostringstream os;
    os << "element '" << std::string(tag.toLatin1()) << "' has no sibling";
    throw com::BadStreamFormat(os.str());
  }
  return e;
}
 ...
 */
calc::WlDelftHabitat::WlDelftHabitat(const com::PathName &xmlFile) : d_xmlFile(xmlFile)
{
...
void calc::WlDelftHabitat::parseParameter(const QDomElement &de)
{
...
void calc::WlDelftHabitat::parseModel(const QDomElement &de)
{
  XML_TRACE_LOG(std::cout << "Begin Model:" << expectAttr(de, "name") << "\n");
  parseRules(de, "Rule");
  parseRules(de, "UITVOERFUNCTIE");
}

void calc::WlDelftHabitat::parseRules(const QDomElement &model, const std::string &tagName)
{
...
void calc::WlDelftHabitat::parseXml()
{
  try {  //  catch xml processing errors
    ...
    ParseHandler parseCommands[] = {{"Parameter", &WlDelftHabitat::parseParameter},
...
calc::WlDelftHabitat::parameter(const std::string &reference) const
{
...
const calc::WlDelftHabitat::Parameter &calc::WlDelftHabitat::output(const QDomElement &rule) const
{
  return (parameter(attrOfSubMatch(rule, "UITVOER", "reference")));
}

const calc::WlDelftHabitat::Parameter &calc::WlDelftHabitat::input(const QDomElement &rule) const
{
  return (parameter(attrOfSubMatch(rule, "INVOER", "reference")));
}

void calc::WlDelftHabitat::addAssignment(const Parameter &assignTo, const std::string &expr)
{
  XML_TRACE_LOG(std::cout << assignTo.d_name << " = " << expr << "\n");
  addFieldAssignment(assignTo.d_name, addFieldExpr(expr), assignTo.d_report);
}

void calc::WlDelftHabitat::addExpr(const QDomElement &rule, const std::string &tagWithExpr)
{
...
static LookupRecord parseRecord(const QDomElement &re, const std::string &keyName,
                                const std::string &resultName)
{
  ...
  try {
    for (auto &i : keys) {
      key.push_back(com::createIntervalFromLookupTableKey<double>(value(i)));
...
static std::vector<QDomElement> nonEmptyContainer(const QDomElement &containedIn,
...
static void parseTable(LookupTable::Records &lr, const QDomElement &containedIn,
                       const std::string &containerName, const std::string &keyName,
                       const std::string &resultName)
{
 ...
 *  a priori
 */
std::string calc::WlDelftHabitat::generatedName(const std::string &nameSuggestion)
{
  // create new name
 ...
 */
std::string calc::WlDelftHabitat::addInlineTable(const std::string &nameSuggestion, LookupTable *tab)
{
...
void calc::WlDelftHabitat::addRuleTableExpr(const QDomElement &rule, const std::string &function,
                                            LookupTable *tab)
{
  std::string const tableName = addInlineTable(output(rule).d_name, tab);
...
calc::StatTable *calc::WlDelftHabitat::addStatistics(const std::string &subjectName)
{
...
void calc::WlDelftHabitat::addKlassen(const QDomElement &rule)
{
  XML_TRACE_LOG(std::cout << "Begin Klassen\n");
  LookupTable::Records lr;
  parseTable(lr, rule, "KlassenTabel", "VARIABELE", "FACTOR");
  addRuleTableExpr(rule, "lookupscalar", fillTable(new LookupTable(VS_S), lr, VS_N));
  XML_TRACE_LOG(std::cout << "End Klassen\n");
}

void calc::WlDelftHabitat::addLookuplinear(const QDomElement &rule)
{
  XML_TRACE_LOG(std::cout << "Begin GebrokenLineair\n");
  LookupTable::Records lr;
  parseTable(lr, rule, "GebrokenLineair", "X", "Y");
  addRuleTableExpr(rule, "lookuplinear", fillTable(new LookupLinear(VS_S), lr, VS_S));
 ...
 */
void calc::WlDelftHabitat::parseEcotoop(const QDomElement &ecotoop)
{
  ...
  std::string const tableName =
      addInlineTable(output.d_name, fillTable(new LookupTable(VS_N), lr, VS_S));
  expr += tableName + ",";
...
void calc::WlDelftHabitat::parseStatistics(const QDomElement &de)
{
  ...
  if (e.isNull() || value(e) != "true") {  // Optional
    ...
    maskCmd << maskedMap << " = "
            ...
            << " and xcoordinate(1) <=" << numericValueOfSubMatch(de, "ZoomXmax") << " then "
            << onderwerp.d_name << ")";
