...
BOOST_AUTO_TEST_CASE(seq_0123_etc)
{
  ...
  }
  {
    ...
    BOOST_TEST(buf[0] == -1.0);
    BOOST_TEST(buf[1] == -0.5);
    BOOST_TEST(buf[4] == 1.0);
  }
  {
    double buf[6];
  ...
  }
  {
    ...
    BOOST_TEST(buf[0] == -2.0);
    BOOST_TEST(buf[1] == -1.0);
    BOOST_TEST(buf[4] == 2.0);
  }
}
...
BOOST_AUTO_TEST_CASE(find_value)
{
  ...
  BOOST_TEST(fStr.find(str, std::string("key")) == "xx");

  str.insert(std::make_pair(std::string("KEY1"), std::string("VAL1")));
  BOOST_TEST(fStr.find(str, std::string("KEY1")) == "VAL1");

  std::map<std::string, double> dob;
  FindValue<std::string, double> dStr(4);
  BOOST_TEST(dStr.find(dob, std::string("key")) == 4);
  dob.insert(std::make_pair(std::string("KEY1"), 1));
  BOOST_TEST(dStr.find(dob, std::string("KEY1")) == 1);
}
