...
*/
com::App::App(int argc, char **argv, const CommandLine &cmdLine, License license)

    : d_argc(argc), d_argv(argv), d_helpArg('h', "help", "print usage information", false),
      d_versionArg('v', "version", "print version information", false),
      d_licenseArg("license", "print license information", false),
#ifdef DEBUG_DEVELOP
      d_debugStreamArg("debug", "debug.out", "file name for debug output", false),
#endif
      d_cmdLine(cmdLine), d_parseCommandLine(true), d_license(license)
...
void com::App::setParseCommandLine(bool setting)
{
  ...
  try {
    ...
    if (d_parseCommandLine) {
      ...
      if (d_helpArg.isParsed()) {
        ...
        stream << d_cmdLine.name() << std::string(" ") << version() << std::string("\n\n");
