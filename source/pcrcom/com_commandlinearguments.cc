...
struct IsWillingToParse {
  ...
  IsWillingToParse(const char *token)
      : std::unary_function<CommandLineArgument *, bool>(), d_token(token)
  {
    POSTCOND(token);
  }
...
*/
com::CommandLineArguments::CommandLineArguments(const CommandLineArguments &arguments)
...
*/
com::CommandLineArguments &com::CommandLineArguments::operator=(const CommandLineArguments &rhs)
{
  if (this != &rhs) {
...
bool com::CommandLineArguments::isDefaultArgument(const CommandLineArgument *arg) const
{
...
*/
void com::CommandLineArguments::add(CommandLineArgument *argument, bool isDefaultArgument)
{
  PRECOND(argument);
  ...
  for (size_t i = 0; i < argc; i += nrTokensParsed) {
    ...
    if (it == d_arguments.end()) {
      if (com::CommandLineArgument::isOption(argv[i])) {
        ...
        stream << "Command line argument '" << argv[i] << "': Invalid option";
        throw com::CommandLineException(stream.str());
      } else {
        std::ostringstream stream;
        stream << "Command line argument '" << argv[i] << "': Argument too much";
        throw com::CommandLineException(stream.str());
      }
    }

    try {
      nrTokensParsed = (*it)->parse(static_cast<size_t>(argc - i), argv + i);
    } catch (com::Exception &exception) {
      std::ostringstream stream;
      stream << "Command line argument '" << *(argv + i) << "': Error";
      exception.prepend(stream.str());
      throw com::CommandLineException(exception);
    } catch (std::range_error & /* e */) {
      std::ostringstream stream;
      stream << "Command line argument '" << *(argv + i) << "': Range error";
      throw com::CommandLineException(stream.str());
    }
  }
}
...
void com::CommandLineArguments::printDescription(std::ostream &stream, size_t offset, size_t width) const
{
  /*
  // Non-positionals.
  for(const_iterator it = d_arguments.begin(); it != d_arguments.end(); ++it) {
...
com::CommandLineArguments::const_iterator com::CommandLineArguments::begin() const
{
  return d_arguments.begin();
}

com::CommandLineArguments::const_iterator com::CommandLineArguments::end() const
{
