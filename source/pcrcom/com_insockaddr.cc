...
com_InSockAddr::com_InSockAddr(const std::string &host, const std::string &service)
{
  struct servent *sp;
  struct hostent *hp;
  ...
  if ((sp = getservbyname(service.c_str(), "tcp")) == NULL) {
    std::string msg = com::createMessage(
        "unable to provide service information for service %s and tcp protocol", service.c_str());
  ...
  if ((hp = gethostbyname(host.c_str())) == NULL) {
    std::string msg =
        com::createMessage("unable to translate host name %s into IP number", host.c_str());
    throw com::Exception(msg);
  }
