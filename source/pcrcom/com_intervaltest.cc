...
BOOST_AUTO_TEST_CASE(round_error)
{
  ...
  // ! v in < 0.2, 0.4]
  //   v in < 0  , 0.2]
  BOOST_CHECK(!BetweenLimits<>(GreaterThan<>(0.2), LessThanEqualTo<>(0.4)).valid(v));
  BOOST_CHECK(BetweenLimits<>(GreaterThan<>(0), LessThanEqualTo<>(0.2)).valid(v));
  {
    typedef float CT;  // choose template
    BOOST_CHECK(!BetweenLimits<CT>(GreaterThan<CT>(0.2F), LessThanEqualTo<CT>(0.4F)).valid(vf));
    BOOST_CHECK(BetweenLimits<CT>(GreaterThan<CT>(0.0F), LessThanEqualTo<CT>(0.2F)).valid(vf));
  }
  {
  ...
  }
  {
    ...
    typedef double CT;  // choose template
    BOOST_CHECK(!BetweenLimits<CT>(GreaterThan<CT>(vf), LessThanEqualTo<CT>(0.4)).valid(vf));
    BOOST_CHECK(BetweenLimits<CT>(GreaterThan<CT>(0), LessThanEqualTo<CT>(vf)).valid(vf));
  }
}
