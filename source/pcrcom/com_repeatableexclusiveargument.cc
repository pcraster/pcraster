...
com::RepeatableExclusiveArgument<ArgumentType, CollectionType>::RepeatableExclusiveArgument(
    const std::string &description, bool isRequired, const ArgumentType &argument1,
    const back_inserter &inserter1, const ArgumentType &argument2, const back_inserter &inserter2)

    : CommandLineArgument(description, isRequired), d_nrParsedArguments(0)
...
com::RepeatableExclusiveArgument<ArgumentType, CollectionType>::RepeatableExclusiveArgument(
    const std::string &description, bool isRequired, const ArgumentType &argument1,
    const back_inserter &inserter1, const ArgumentType &argument2, const back_inserter &inserter2,
    const ArgumentType &argument3, const back_inserter &inserter3, const ArgumentType &argument4,
...
*/
template <class ArgumentType, class CollectionType>
void com::RepeatableExclusiveArgument<ArgumentType, CollectionType>::addArgument(
...
template <class ArgumentType, class CollectionType>
size_t com::RepeatableExclusiveArgument<ArgumentType, CollectionType>::parse(size_t argc,
                                                                             char *const *argv)
{
  ...
  while (1) {
    ...
    for (typename Inserters::iterator it = d_inserters.begin(); it != d_inserters.end(); ++it) {
...
template <class ArgumentType, class CollectionType>
bool com::RepeatableExclusiveArgument<ArgumentType, CollectionType>::isParsed() const
{
  return d_nrParsedArguments > 0;
}

template <class ArgumentType, class CollectionType>
bool com::RepeatableExclusiveArgument<ArgumentType, CollectionType>::canParse(const char *token) const
{
  // See if one of the possible arguments can parse the token.
  for (typename Inserters::const_iterator it = d_inserters.begin(); it != d_inserters.end(); ++it) {
...
*/
template <class ArgumentType, class CollectionType>
void com::RepeatableExclusiveArgument<ArgumentType, CollectionType>::printSynopsis(
    std::ostream &stream) const
{
  if (!d_inserters.empty()) {

    if (d_inserters.size() > 1) {
      for (typename Inserters::const_iterator it = d_inserters.begin(); it != d_inserters.end() - 1;
           ++it) {
        stream << " (";
...
template <class ArgumentType, class CollectionType>
void com::RepeatableExclusiveArgument<ArgumentType, CollectionType>::printDescription(
    std::ostream &stream, size_t offset, size_t width) const
{
