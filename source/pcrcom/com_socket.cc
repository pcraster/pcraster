...

{
  ...
  if ((p = ::getprotobyname(proto.c_str())) == NULL) {
    std::string msg = com::createMessage("unknown prototype: %s", proto.c_str());
...
#ifdef BORLANDC
  if (::bind(d_fd, (struct sockaddr *)&addr.getAddress(), sizeof(addr.getAddress())) == SOCKET_ERROR) {
    throw com::Exception("unable to bind socket to internet address");
  }
#else
  if (::bind(d_fd, (struct sockaddr *)&addr.getAddress(), sizeof(addr.getAddress())) < 0) {
    throw com::Exception(::strerror(errno));
  }
#endif
}
...
#ifdef BORLANDC
  if ((fd = ::accept(socket, (struct sockaddr *)&theirAddress, &size)) == INVALID_SOCKET) {
