...
std::string com::createMessage(const char *format, ...)
{
  ...
  std::vsnprintf(message, buf_size, format, arguments);
  va_end(arguments);
 ...
 */
bool com::equalNoCase(const std::string &s, const std::string &s2)
{
...
void com::removeFrontEndString(std::string &str, const std::string &strToRemove)
{
  ...
  if (!strToRemove.empty()) {
    while (str.length() >= strToRemove.length() && str.substr(0, strToRemove.length()) == strToRemove) {
      str.erase(0, strToRemove.length());
    }

    while (str.length() >= strToRemove.length() &&
           str.substr(str.length() - strToRemove.length(), strToRemove.length()) == strToRemove) {
 ...
 *   \endcode
 */
void com::replaceChars(std::string &str, char resultChar, const std::string &charsToBeReplaced)
{
 ...
 *   \endcode
 */
std::string com::replaceCharByStr(const std::string &str, char c, const std::string &with)
{
  std::string d;
  for (char const i : str) {
    if (i == c) {
