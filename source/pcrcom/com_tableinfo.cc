...
private:
  ...
  void parsePlainText()
  {
    ...
    d_values.clear();
    sfp.pi = boost::spirit::parse(
        sfp.begin(), sfp.end(),
        while_p(std::bind(
            &com::TableInfoParser::continueParsing,
            this))[*(eol_p[std::bind(&com::TableInfoParser::endLine, this)]) >>  // allow new lines
                   // some values on a line, 0 is ok, empty file
                   +(real_p[append(d_values)]) >>
                   (+(eol_p[std::bind(&com::TableInfoParser::endLine, this)]) |
                    end_p[std::bind(&com::TableInfoParser::endLine,
                                    this)])  // terminated by some new lines or eof
    ]                                        // [ handler() ]
        ,
        blank_p);
    d_description.clear();
    ...
    sfp.pi = boost::spirit::parse(
        ...
        lexeme_d[*(anychar_p[append(d_description)] - eol_p)] >> eol_p >>
            ...
            while_p(std::bind(&com::TableInfoParser::columnNamesToParse, this))[(
                (*(anychar_p - eol_p))[std::bind(&com::TableInfoParser::add, this, std::placeholders::_1,
                                                 std::placeholders::_2)] >>
                eol_p)] >>
...
public:
  TableInfoParser(const com::PathName &pn) : d_pn(pn)
  {
...
com::TableInfo::TableInfo(const com::PathName &fileName)
    : d_fileName(fileName), d_columnNamesGenerated(false)
{
