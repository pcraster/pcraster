 ...
 */
geo::BandMap::BandMap(const com::PathName &pn, const RasterSpace &rs, CSF_CR cellRepr, bool mvIsSet,
                      double mvValue)
    : d_pn(pn), d_nrRows(rs.nrRows()), d_nrCols(rs.nrCols()), d_nrBands(1),
...
geo::BandMap::BandMap(const com::PathName &pn, bool allowUpdate)
    : d_pn(pn), d_nrRows(0), d_nrCols(0),  // means not set
      d_nrBands(1), d_cellRepr(CR_UINT1), d_hasHostByteOrder(true), d_layout(BIL),
      d_skipBytes(0),  // d_bandGapBytes(0),
      d_cellSize(1), d_ulXMap(0), d_mvIsSet(false)
{
  initPathName();
  open(allowUpdate);
}
...
template <>
void createBil<UINT1>(const com::PathName &pn, const RasterSpace &rs, const UINT1 *data, UINT1 mvValue)
{
...
template <> void createBil<UINT1>(const com::PathName &pn, const RasterSpace &rs, const UINT1 *data)
{
...
template <>
void createBil<INT4>(const com::PathName &pn, const RasterSpace &rs, const INT4 *data, INT4 mvValue)
{
...
template <> void createBil<INT4>(const com::PathName &pn, const RasterSpace &rs, const INT4 *data)
{
...
template <>
void createBil<REAL4>(const com::PathName &pn, const RasterSpace &rs, const REAL4 *data, REAL4 mvValue)
{
...
template <> void createBil<REAL4>(const com::PathName &pn, const RasterSpace &rs, const REAL4 *data)
{
