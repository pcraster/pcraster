...
geo::CellLocVisitor::CellLocVisitor(size_t nrRows, size_t nrCols)
    : d_rd(nrRows, nrCols), d_currentCell(0, 0)
{
}

geo::CellLocVisitor::CellLocVisitor(const RasterDim &rd) : d_rd(rd), d_currentCell(0, 0)
{
}
...
geo::CellLoc geo::CellLocVisitor::downstream(unsigned int lddVal) const
{
  ...
  return {row() + lddDirection[lddVal].deltaRow, col() + lddDirection[lddVal].deltaCol};
}

//! checked version
bool geo::CellLocVisitor::downstream(CellLoc &cl, unsigned int lddVal) const
{
