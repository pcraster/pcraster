...
geo::CSFStack<T>::CSFStack(std::string const &name, CSF_CR useType)
    ...
    : d_name(CSFStackName(name)), d_step(-1), d_minIsValid(false), d_maxIsValid(false),
...
geo::CSFStack<T>::CSFStack(const geo::CSFStackName &name, CSF_CR useType)
    ...
    : d_name(name), d_step(-1), d_minIsValid(false), d_maxIsValid(false), d_valueScale(VS_UNDEFINED),
      d_useType(useType), d_raster(nullptr)
...
geo::CSFStack<T>::CSFStack(const CSFStack &rhs)
    ...
    : d_name(rhs.d_name), d_step(rhs.d_step), d_min(rhs.d_min), d_minIsValid(rhs.d_minIsValid),
      d_max(rhs.d_max), d_maxIsValid(rhs.d_maxIsValid), d_valueScale(rhs.d_valueScale),
      d_useType(rhs.d_useType), d_raster(nullptr)
...
template <class T> geo::CSFStack<T>::~CSFStack()
{
  clean();
}

template <class T> void geo::CSFStack<T>::clean()
{
  delete d_raster;
  d_raster = nullptr;
}

template <class T> void geo::CSFStack<T>::init()
{
  ...
  try {
    ...
    std::unique_ptr<CSFMap> map = std::make_unique<CSFMap>(fn);  // READ
    ...
    d_raster =
        new Raster<T>(map->nrRows(), map->nrCols(), map->cellSize(), map->left(), map->top(), proj);
...
*/
template <class T> void geo::CSFStack<T>::load(size_t t)
{
...
*/
template <class T> bool geo::CSFStack<T>::loaded(size_t t) const
{
  if (d_step < 0) {
    return false;
...
/*!
  ...
  \sa        loaded(size_t)
*/
template <class T> bool geo::CSFStack<T>::isAvailable(size_t t) const
{
...
/*!
  ...
  \sa        firstStep()
*/
template <class T> size_t geo::CSFStack<T>::lastStep() const
{
...
*/
template <class T> const geo::Raster<T> &geo::CSFStack<T>::raster() const
{
  return *d_raster;
}
...
/*!
  ...
  \sa        max()
*/
template <class T> bool geo::CSFStack<T>::min(T *m) const
{
  *m = d_min;
  return d_minIsValid;
}
...
/*!
  ...
  \sa        min()
*/
template <class T> bool geo::CSFStack<T>::max(T *m) const
{
  *m = d_max;
  return d_maxIsValid;
}
...
*/
template <class T> bool geo::CSFStack<T>::allMV() const
{
...
*/
template <class T> const geo::CSFStackName &geo::CSFStack<T>::name() const
{
  return d_name;
}

template <class T> const geo::RasterSpace *geo::CSFStack<T>::rasterSpace() const
{
...
/*!
  \return    Value scale.
*/
template <class T> CSF_VS geo::CSFStack<T>::valueScale() const
{
  return d_valueScale;
}
...
/*!
  ...
  \sa        legend()
*/
template <class T> bool geo::CSFStack<T>::hasLegend() const
{
...
/*!
  ...
  \sa        hasLegend()
*/
template <class T> const com::Legend<INT4> *geo::CSFStack<T>::legend() const
{
