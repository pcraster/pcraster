...
geo::KernelSourceBuffer<T, U>::KernelSourceBuffer(const SimpleRaster<T> &raster, size_t radius,
                                                  size_t border)

    : SimpleRaster<U>(2 * radius + 1, raster.nrCols() - 2 * border), d_raster(raster), d_radius(radius),
      d_border(border), d_row(border + radius)

{
  PRECOND(radius > 0);
  PRECOND(d_raster.nrRows() >= 2 * d_radius + 1 + 2 * d_border);
  PRECOND(d_raster.nrCols() >= 2 * d_radius + 1 + 2 * d_border);
  PRECOND(static_cast<int>(d_raster.nrRows()) - 2 * static_cast<int>(d_border) -
...
/*!
*/
template <class T, class U> geo::KernelSourceBuffer<T, U>::~KernelSourceBuffer()
{
}

template <class T, class U> size_t geo::KernelSourceBuffer<T, U>::border() const
{
  return d_border;
}
...
*/
template <class T, class U> bool geo::KernelSourceBuffer<T, U>::advanceRow()
{
  ...
  if (d_row + 1 + d_radius > d_raster.nrRows() - 1 - d_border) {
    ...
    update(d_border, d_row + d_radius, this->nrCols(), d_row + d_radius, 0, 2 * d_radius);
    return true;
  }
}
...
*/
template <class T, class U> size_t geo::KernelSourceBuffer<T, U>::row() const
{
  return d_row;
}
...
/*!
  ...
  \sa        row()
*/
template <class T, class U> const geo::SimpleRaster<T> &geo::KernelSourceBuffer<T, U>::raster() const
{
  return d_raster;
}
...
*/
template <class T, class U> size_t geo::KernelSourceBuffer<T, U>::nrRowsToProcess() const
{
...
*/
template <class T, class U> size_t geo::KernelSourceBuffer<T, U>::kernelRadius() const
{
  return d_radius;
}
