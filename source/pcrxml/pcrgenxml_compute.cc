...
//! ctor
pcrxml::Compute::Compute(const QDomElement &element)
...
pcrxml::Compute::Compute(const Compute &src)
    : pcrxml::Element(src), maximalMigrationDistance(src.maximalMigrationDistance),
      maximumZeroDownCrossingThreshold(src.maximumZeroDownCrossingThreshold),
      minimalSlopeBrinkPoint(src.minimalSlopeBrinkPoint), minimalBrinkHeight(src.minimalBrinkHeight)
{
}
