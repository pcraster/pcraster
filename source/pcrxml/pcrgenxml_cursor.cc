...
//! ctor
pcrxml::Cursor::Cursor(const QDomElement &element)
    : Element(element, d_elementName), x(element, "x", true), y(element, "y", true),
      t(element, "t", true)
{
}
...
//! copy ctor
pcrxml::Cursor::Cursor(const Cursor &src) : pcrxml::Element(src), x(src.x), y(src.y), t(src.t)
{
}
