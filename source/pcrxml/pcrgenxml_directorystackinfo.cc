...
//! ctor
pcrxml::DirectoryStackInfo::DirectoryStackInfo(const QDomElement &element)
    : Element(element, d_elementName), allMissingValue(element, "allMissingValue", false),
      minimumValue(element, "minimumValue", true), maximumValue(element, "maximumValue", true),
      stackEnd(element, "stackEnd", false), dataTypeDTD(element, "dataTypeDTD", true)
{
}
...
pcrxml::DirectoryStackInfo::DirectoryStackInfo(const DirectoryStackInfo &src)
    : pcrxml::Element(src), allMissingValue(src.allMissingValue), minimumValue(src.minimumValue),
      maximumValue(src.maximumValue), stackEnd(src.stackEnd), dataTypeDTD(src.dataTypeDTD)
{
}
