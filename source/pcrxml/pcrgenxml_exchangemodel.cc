...
pcrxml::ExchangeModel::ExchangeModel(const ExchangeModel &src)
    : pcrxml::Element(src), id(src.id), ioStrategy(src.ioStrategy), description(src.description), integerTimer(((src.integerTimer) != nullptr) ? new IntegerTimer(*(src.integerTimer)) : nullptr), areaMapDTD(((src.areaMapDTD) != nullptr) ? new AreaMapDTD(*(src.areaMapDTD)) : nullptr)
{
