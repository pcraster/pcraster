...
//! ctor
pcrxml::IntegerTimer::IntegerTimer(const QDomElement &element)
    : Element(element, d_elementName), start(element, "start", true), end(element, "end", true),
...
pcrxml::IntegerTimer::IntegerTimer(const IntegerTimer &src)
    : pcrxml::Element(src), start(src.start), end(src.end), step(src.step)
{
}
