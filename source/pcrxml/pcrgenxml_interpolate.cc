...
//! ctor
pcrxml::Interpolate::Interpolate(const QDomElement &element)
    : Element(element, d_elementName), searchRadius(element, "searchRadius", true),
...
pcrxml::Interpolate::Interpolate(const Interpolate &src)
    : pcrxml::Element(src), searchRadius(src.searchRadius), mapCellSize(src.mapCellSize),
