...
//! ctor
pcrxml::ModelRunChild::ModelRunChild(const QDomElement &element)
    : Element(element, d_elementName), directoryName(element, "directoryName", true)
{
}
...
pcrxml::ModelRunChild::ModelRunChild(const ModelRunChild &src)
    : pcrxml::Element(src), directoryName(src.directoryName)
{
}
