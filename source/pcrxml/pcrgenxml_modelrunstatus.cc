...
//! ctor
pcrxml::ModelRunStatus::ModelRunStatus(const QDomElement &element)
    : Element(element, d_elementName), userInterfaceName(element, "userInterfaceName", false),
      userCanChange(element, "userCanChange", true), userCanRun(element, "userCanRun", true),
      userCanDelete(element, "userCanDelete", true), userDerive(element, "userDerive", true),
      resultLastRun(element, "resultLastRun", true), lastTimeStep(element, "lastTimeStep", false)
{
}
...
pcrxml::ModelRunStatus::ModelRunStatus(const ModelRunStatus &src)
    : pcrxml::Element(src), userInterfaceName(src.userInterfaceName), userCanChange(src.userCanChange),
      userCanRun(src.userCanRun), userCanDelete(src.userCanDelete), userDerive(src.userDerive),
      resultLastRun(src.resultLastRun), lastTimeStep(src.lastTimeStep)
{
}
