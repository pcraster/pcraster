...
//! ctor
pcrxml::NumericSetting::NumericSetting(const QDomElement &element)
    : Element(element, d_elementName), name(element, "name", true), value(element, "value", true)
{
}
...
pcrxml::NumericSetting::NumericSetting(const NumericSetting &src)
    : pcrxml::Element(src), name(src.name), value(src.value)
{
}
