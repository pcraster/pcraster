...
//! ctor
pcrxml::TimestepRange::TimestepRange(const QDomElement &element)
    : Element(element, d_elementName), first(element, "first", true), last(element, "last", true)
{
}
...
pcrxml::TimestepRange::TimestepRange(const TimestepRange &src)
    : pcrxml::Element(src), first(src.first), last(src.last)
{
}
