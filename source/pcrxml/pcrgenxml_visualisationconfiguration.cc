...
//! ctor
pcrxml::VisualisationConfiguration::VisualisationConfiguration(const QDomElement &element)
    : Element(element, d_elementName), date(element, "date", true), version(element, "version", true),
      cwd(element, "cwd", true), os(element, "os", true)
{
  try {
...
//! assignment operator
pcrxml::VisualisationConfiguration &
pcrxml::VisualisationConfiguration::operator=(const VisualisationConfiguration &src)
{
  if (this != &src) {
    clean();
    PRECOND(false);
