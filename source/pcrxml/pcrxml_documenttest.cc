...
BOOST_AUTO_TEST_CASE(pcr_document)
{
  ...
  {  // copy from element tree, do not add xmlns attrs
    Document const doc(
        pcrxml::createPcrDocument("<ExchangeModel xmlns='XX' attr='value'></ExchangeModel>"));
  ...
  {  // copy from element tree
    Document const doc(pcrxml::createPcrDocument("<ExchangeModel><TT x='v'/><X>de</X></ExchangeModel>"));
    const char *cmp =
        "<?xml version='1.0' encoding='ISO-8859-1' standalone='yes'?>"
        "<!DOCTYPE ExchangeModel PUBLIC '-//PCRaster//Generic' 'pcraster.dtd'>"
...
BOOST_AUTO_TEST_CASE(ctor_and_parser)
{
  ...
  try {
    ...
    Document const doc6("<systemLicense><licensee value='Piet Snot'/></systemLicense>");
