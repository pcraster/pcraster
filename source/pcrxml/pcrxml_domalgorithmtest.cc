...
BOOST_AUTO_TEST_CASE(for_each_node)
{
  ...
  {
   ConcatNodeValues ca;
   Document const doc("<Main><S1 />Text<S2 b='a2' a='a1'/><S3><S1 a='a3' /></S3></Main>");
   forEachNode(doc.documentElement(), ca);
   BOOST_CHECK(ca.val == "Texta1a2a3");
  }
  {
   ConcatNodeValues ca;
   Document const doc("<Main><S1 />Text<S2 a='a1' b='a2'/><S3><S1 a='a3' /></S3></Main>");
   forEachNode(doc.documentElement(), ca);
   BOOST_CHECK(ca.val == "Texta1a2a3");
  }
}
...
BOOST_AUTO_TEST_CASE(for_each_element)
{
 ...
 Document const doc("<?jsx version='1'?><Main><S1 /><S2 b='a2'/><S3><S1 a='a2' /></S3></Main>");
...
BOOST_AUTO_TEST_CASE(for_each_child_element)
{
 ...
 CountNodeAttrs ca;
 Document const doc("<S1 b='notCountedAsChild'><S1 /><S2 b='a2'/><S3><S1 a='a2NotCountedAsChild' /></S3></S1>");
 ...
 QDomElement const e(doc.documentElement());
