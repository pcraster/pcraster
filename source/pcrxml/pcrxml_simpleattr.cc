...
template <> size_t fromString<size_t>(const std::string &str)
{
  return com::strToSize_t(str);
}

template <> int fromString<int>(const std::string &str)
{
    ...
    : Attribute(owningElement, name, required)
{
  ...
  try {
    ...
    throw com::BadStreamFormat(asString(owningElement.nodeName()) + ":" + name +
                               ": invalid attribute value");
  }
}

//! ctor from value
template <class T> pcrxml::SimpleAttr<T>::SimpleAttr(T value) : Attribute(true), d_value(value)
{
}

//! ctor
template <class T> pcrxml::SimpleAttr<T>::SimpleAttr() : Attribute(false)
{
}
...
//! Assignment operator.
template <class T> pcrxml::SimpleAttr<T> &pcrxml::SimpleAttr<T>::operator=(const SimpleAttr &rhs)
{
  if (this != &rhs) {
    if (rhs.present()) {
      setPresent(true);
...
//! dtor
template <class T> pcrxml::SimpleAttr<T>::~SimpleAttr()
{
}

template <class T> std::string pcrxml::SimpleAttr<T>::attrValueStr() const
{
