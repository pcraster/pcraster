         ...
         char *argv[]) /* list of arguments */
{
  ...
  if (opB == 1 || opMV == 1) {
    ...
    if (opB == 1) {
      if (SmallestFittingRectangle(&X0, &Y0, &nrRows, &nrCols, in, borderval, nrMaps, cellSize, angleIn,
                                   projection, contract)) {
        FreeMaps(in, nrMaps);
        AppEnd();
        exit(1);
      ...
      if (SmallestNonMVRect(&X0, &Y0, &nrRows, &nrCols, in, borderval, nrMaps, valueScale, cellSize,
