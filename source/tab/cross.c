...
static LOOK_UP_TABLE *MakeHistoCross(MAP *m, size_t nrInt, size_t nrSlots)
{
  ...
  if (vs == VS_DIRECTION && h->records[0][INTERVAL].l == -1) { /* keep single entry for flat */
  ...
  for (; h_i < h->nrRecords; h_i++) {
    ...
    if (total >= chunk * (r + 1) || (h_i == h->nrRecords - 1)) { /* interval filled or last one */
  ...
  h->records = (LOOK_UP_KEY **)Realloc2d((void **)h->records, r, (size_t)2, h->nrRecords, (size_t)2,
                                         sizeof(LOOK_UP_KEY));
