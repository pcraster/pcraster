 ...
 */
typedef LOOK_UP_KEY **(*SEARCH)(const void *key, const void *base, size_t nmemb, size_t size,
                                QSORT_CMP cmp);
...
static int CmpKey(const double *keyValues, const LOOK_UP_KEY **keys)
{
  ...
  const QSORT_CMP Cmp[] = {(QSORT_CMP)TestOne,   (QSORT_CMP)TestInfInf, (QSORT_CMP)TestGeInf,
                           ...
                           (QSORT_CMP)TestGtLe,  (QSORT_CMP)TestInfLt,  (QSORT_CMP)TestGeLt,
                           (QSORT_CMP)TestGtLt};
  PRECOND(cmpNrKeys > 0);
 ...
 */
LOOK_UP_KEY *FindLookupKey(const LOOK_UP_TABLE *t,  /* lookup table */
                              ...
                              */
{
  ...
  cmpNrKeys = t->nrKeys;
  if (t->searchMethod == 0) {
    k = lfind(keyValues, t->records, &nr, sizeof(LOOK_UP_KEY *), (QSORT_CMP)CmpKey);
  } else {
...
size_t FindCrossKey(const LOOK_UP_TABLE *t,  /* lookup table */
                    const double *keyValues, /* values to match, no MV's allowed
                              */
                              ...
                              */
{
  ...
  cmpNrKeys = t->nrKeys;
  if (t->searchMethod == 0) {
    k = lfind(keyValues, t->records + startHere, &nr, sizeof(LOOK_UP_KEY *), (QSORT_CMP)CmpKey);
  } else {
