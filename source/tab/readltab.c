...
static int SetNumber(LOOK_UP_KEY *k, bool low, CSF_VS vs)
{
...
static int IllegalState(LOOK_UP_KEY *k, int token, const char *legals) /* number must be first one ! */
{
  k->t = TEST_ERROR;
  if (!LexError(token)) {
        ...
        switch (t) {
          case LEX_NUMBER:
            ...
            if (k->t != TEST_INF_INF && (k->l > k->h)) {
              ...
              return RetErrorNested(1, "low value ('%g') of range larger than high value ('%g')", k->l,
                                    k->h);
            }
            break;
          case ']':
...
static size_t DetectNrColsTable(FILE *tableFile) /* file to read */
{
  LOOK_UP_KEY k;
  long l = 0;
  int nrCols = 0;
 ...
 * in ErrorNested
 */
static LOOK_UP_KEY *ReadLookupRecs(size_t *nrCols,     /* write-only */
                                   ...
                                   FILE *tableFile)
{
  ...
  for (nrK = n = 0; /* break from code */; n++) {
    ...
    if (vsTargetVal != VS_UNDEFINED && (n % c) == nrKeysExp && k[n].t != TEST_ONE) {
      ErrorNested("value field at line '%ld' column '%d' is not a single value", LexGetLineNr(),
                  (n % c) + 1);
      goto error;
    }
    if ((n % c) == 0) /* start new record */
    {
      ...
      {
        ErrorNested("Too many columns on line '%ld', expected '%d' columns", recStartAt, c);
        goto error;
      }
    ...
    } else /* add to this record */
    {
      ...
      {
      notEnough:
        ErrorNested("Not enough columns on line '%ld', expected '%d' columns", recStartAt, c);
        goto error;
      }
    }
  } /* eofor */
...
static int CheckMatrixCell(size_t row, /* 1-based index */
 ...
 */
LOOK_UP_TABLE *ReadLookupTable(FILE *f,             /* the tablefile */
                               ...
                               CSF_VS outputVs)     /* VS_UNDEFINED if we don't care */
{
  ...
  k = ReadLookupRecs(&nrCols, &nrRecs, nrKeys, matrRead ? VS_UNDEFINED : outputVs, f);
  ...
  if (matrRead) { /* build lookup from matrix and check matrix output/target values */
    size_t i = 0;
