 ...
 */
static void *InsertSorted(const void *key, /* key to be inserted */
...
void STforAll(SEARCH_TABLE *t,   /* table to deallocate */
              ACTION_REC action) /* */
{
  size_t i = 0;
...
void *STsearch(const SEARCH_TABLE *t, /* table to search */
               SEARCH_REC f)          /*  */
{
  size_t i = 0;
  size_t fastBegin = 0;
 ...
 *
 */
SEARCH_TABLE *STnew(size_t nrFastList,  /* nr. of elements in fast list
 ...
 *
 */
void *STinsert(SEARCH_TABLE *t, /* read-write table */
 ...
 *
 */
void *STfind(const SEARCH_TABLE *t, /* read-write table */
             const void *record)    /* record to find */
{
  ...
  if (t->nrFastList != 0) {
    ...
    if (0 <= id && id < (int)t->nrFastList) { /* id is in fast list */
      return (void *)((char *)t->fastList + (id * t->recSize));
    }
  }
...
void *STfindOrInsert(SEARCH_TABLE *t, /* read-write table */
